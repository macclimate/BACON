...
%                           Last modification:  Jun 11, 2008
%

% Revisions:
%
%   June 11, 2008
%     - tipping bucket rain gauge will be used instead of Geonor between
%     Apr 1 and Oct 30.  Geonor is too temperature dependant to relay on it
%     for day to day rain.
%   Apr 21, 2008
...
% Get tipping bucket data
P1       = read_bor(fullfile(pth,'oy_clim1.33'),[],[],yearX);
P2       = read_bor(fullfile(pth,'oy_clim1.34'),[],[],yearX);


% Estimate Geonor daily cummulative value
tv =  tv_GMT - 8/24;
...
% Calculate tipping bucket cummulative value
P1 = P1(ind48);
P2 = P2(ind48);
Ptip = max(sum(P1),sum(P2));

%plot(tv,Pgeonor,tv,[P1 P2]);datetick('x');legend(sprintf('Geonor = %f',precipOut),sprintf('Bucket = %f',Ptip))

% Between Apr 1 and Oct 30 use tipping bucket instead of Geonor
dateX = datevec(now);
doyX = floor(now - datenum(dateX(1),1,0));
if doyX >= 92 & doyX <306
    precipOut = Ptip;
end
