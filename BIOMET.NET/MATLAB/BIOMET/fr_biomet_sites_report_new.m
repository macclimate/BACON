...
%     end
    ...
    for j = 1:size(DATA_MAT,2)
        ...
        % since we are using gap-filled traces, sometimes climate data
        % exist to run the models past the point where we have calculated
        % fluxes. Nuke this part of the plot so we are only are looking at
        % filled traces rather than superfluous gap-filling.
        ind = find(~isnan(DATA_MAT(:,j)) & TV_MAT(:,j) >= datenum(Years(end),1,doy_data));
        DATA_MAT(ind,j)=NaN;
    end
