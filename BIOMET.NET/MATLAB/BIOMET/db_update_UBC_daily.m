...
% Revisions:
% Mar 14, 2018 (Patrick and Zoran)
%   - Added LGR1 site
% Oct 3, 2014  - Nick - add MainEddy processing for SQM (ShellQuest)
% May 9, 2012  - Nick - add MainEddy processing for SQT (ShellQuest)
...
% update current biomet site list, Sept 22, 2015
if ~exist('Sites') | isempty(Sites)
   Sites     = {'BS'   'PA'   'YF'       'SQT'      'SQM'      'LGR1'};
   Site_name = {'PAOB' 'PAOA' 'YF'       'SQT'      'SQM'      'LGR1'};
   Site_eddy = {'Eddy' 'Eddy' 'MainEddy' 'MainEddy' 'MainEddy' 'MainEddy'};
end
...
for i = 1:length(Sites)
   ...
   switch upper(SiteId)
       case {'OY'}
           wc = ['*s.h' SiteId '.mat'];
       case {'HDF11' 'HJP02' 'HJP75' 'SQT' 'SQM' 'LGR1'}
           wc = ['*.h' SiteId '.mat'];
       case {'BS'}
   ...
   case {'HDF11'}
      ...
      % CR ACS system set up at HDF11
      sys = 'ACS-DC';
      wc = ['*.ACS_Flux_CR16.mat'];
      pth_new_mat_files = ['\\paoa001\Sites\' SiteNm '\ACS\hhour'];
      lst_old = dir(fullfile(pth_new_mat_files,wc));
      try
         db_incremental_update(pth_new_mat_files,pth_db,pth_update_lst,wc,sys,SiteId,no_days_back);
      end
