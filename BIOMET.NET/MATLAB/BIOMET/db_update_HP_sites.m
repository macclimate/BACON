...
% file created: June 1, 2010        last modified: July 18, 2011
% (c) Nick Grant
...
for k=1:length(yearIn)
    for j=1:length(sites)
        ...
        if strcmp(char(sites(j)),'09')
            eval(['progressListHP' char(sites(j)) '_Soil30min_Pth = fullfile(pth_db,''HP' char(sites(j))...
            '_Soil30min_progressList_' num2str(yearIn(k)) '.mat'');']);
            eval(['HP' char(sites(j)) 'SoilDatabase_Pth = [pth_db ''yyyy\hp' char(sites(j))...
            '\Climate\Soil\''];']);
        end
        ...
        if strcmp(char(sites{j}),'11') % extra code to handle corrected Flux file from LI-7200
            eval(['progressListHP' char(sites(j)) '_ECcorrFlux_Pth = fullfile(pth_db,''HP' char(sites(j))...
            '_EC_flux_30m_progressList_' num2str(yearIn(k)) '.mat'');']);
            eval(['HP' char(sites(j)) 'ECcorrFlux_Database_Pth = [pth_db ''yyyy\hp'...
             char(sites(j)) '\Flux_Logger\''];']);
        end
       ...
       filePath = sprintf('d:\\sites\\HP%s\\CSI_net\\old\\',char(sites(j)));
        ...
        if strcmp(char(sites(j)),'09')
           ...
           eval(['[numOfFilesProcessed,numOfDataPointsProcessed] = fr_site_met_database(''D:\sites\HP' char(sites(j)) '\CSI_net\old\' num2str(yearIn(k)) '\hp' char(sites(j)) '_Soil.' num2str(yearIn(k)) '*'',[],[],[],progressListHP' char(sites(j)) '_Soil30min_Pth,HP' char(sites(j)) 'SoilDatabase_Pth,2);'])
           eval(['disp(sprintf(''HP' char(sites(j)) ' Soil:  Number of files processed = %d, Number of HHours = %d'',numOfFilesProcessed,numOfDataPointsProcessed))']);
        
        elseif strcmp(char(sites(j)),'11')
            eval(['[numOfFilesProcessed,numOfDataPointsProcessed] = fr_site_met_database(''D:\sites\HP' char(sites(j)) '\CSI_net\old\' num2str(yearIn(k)) '\hp' char(sites(j)) '_Clim.' num2str(yearIn(k)) '*'',[],[],[],progressListHP' char(sites(j)) '_30min_Pth,HP' char(sites(j)) 'ClimatDatabase_Pth,2);'])
           eval(['disp(sprintf(''HP' char(sites(j)) ' Clim:  Number of files processed = %d, Number of HHours = %d'',numOfFilesProcessed,numOfDataPointsProcessed))']);
        end
        ...
        if strcmp(char(sites{j}),'11') % extra code to handle corrected Flux file from LI-7200
           eval(['[numOfFilesProcessed,numOfDataPointsProcessed] = fr_site_met_database(''D:\sites\HP' char(sites(j)) '\CSI_net\old\' num2str(yearIn(k)) '\hp' char(sites(j)) '_eddy_Flux.' num2str(yearIn(k)) '*'',[],[],[],progressListHP' char(sites(j)) '_ECcorrFlux_Pth,HP' char(sites(j)) 'ECcorrFlux_Database_Pth,2);'])
           eval(['disp(sprintf(''HP' char(sites(j)) ' EC_flux_corr:  Number of files processed = %d, Number of HHours = %d'',numOfFilesProcessed,numOfDataPointsProcessed))']);
        end
