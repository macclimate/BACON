...
if runCleaning == 1
    ...
    eval_wrapper('Updating Eddy DB with long hhour files','db_update_hhour_database([yy(1) yy(1)-1]);');
    eval_wrapper('Updating BERMS DB for current year','db_update_BERMS_daily([],[yy(1) yy(1)-1])');
    ...
    if local_export % do daily export to our local copy of the FCRN db (for internal use only!)
        ...
        sites = {'BS' 'PA' 'YF' 'HDF11' 'UBC_TOTEM' 'MPB1' 'MPB2' 'MPB3' 'HP09' 'HP11'}; % removed decommissioned sites: July 20, 2011
        for i=1:length(sites) % run cleaning with export of web graphs
            if ~strcmp(char(sites{i}),'UBC_TOTEM')
               eval_wrapper(['Automatic cleaning of current year for ' sites{i} ],'fr_automated_cleaning([yy(1) yy(1)-1],sites{i},6)');
            else
    ...
    if strcmp(dayOfWeek,'Sat') & yy(3) <= 7
        ...
        eval_wrapper('DIS data export','fr_automated_cleaning(yy(1)-1:yy(1),{''bs'',''cr'',''pa'',''yf'',''oy''},4)'); % removed hjp02: July 20, 2011
