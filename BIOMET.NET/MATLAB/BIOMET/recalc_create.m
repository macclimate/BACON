...
[s,mes] = copyfile([db_PAOA001_root 'matlab'],[base_dir '\Biomet.net\Matlab']);
if ~isempty(mes)
    disp(mes)
    disp('... returning');
    diary off
...
[s,mes] = copyfile([db_PAOA001_root 'Sites\' SiteName '\UBC_PC_SETUP\Site_specific\*.m'],[base_dir '\UBC_PC_SETUP\Site_specific']);
...
%cal files
[s,mes] = copyfile([db_PAOA001_root 'Sites\' SiteName '\hhour\*calibrations*.*'],[base_dir '\hhour']); 
if ~isempty(mes)
    disp(['**************** -> Error: ' mes])
    %disp('... returning');
    %return
