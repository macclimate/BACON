function k = db_main_update_eddy(pthIn,wildcard,pthOut)
%
% eg. k = db_main_update_eddy('\\annex001\database\2000\cr\flux\raw\',
...
%
% (c)                   File created:       May 13, 1998
%                       Last modification:  Jan 18, 2000
%
% Revisions:
%
% Jan 18, 2000
%   - enabled using short form hhour files
%
...
for i=1:n
    ...
    if D(i).isdir == 0        % avoid directories, use short form hhour files (Jan 18, 2000)
        ...
        OutputData = db_update_eddy(x.stats,pthOut);
        k = k+1;
    end
end
tm = toc;
