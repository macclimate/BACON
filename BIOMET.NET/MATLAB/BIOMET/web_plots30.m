...
%                           Last modification:  Jan 3, 2012
%

% Revisions:
% Jan  3, 2012: make it year independent RK
% Jan 17, 2011: 
%     - reduced the font size and line thicknesses
% Jan 11, 2011: new year
% Feb 6, 2008: new year
...
%in_pth = '\\annex001\DataBase\2012\UBC_Totem\Climate\Totem1';
...
start_date = datenum(yearToday-1,12,31);
...
for i = 5:29
   col_id = num2str(i);
   c = sprintf('c%s=read_bor(''%s%s%s'',[],[],[],indOut);',col_id,in_pth,filename,col_id);
   eval(c); 
end
...
for n = 1
   ...
   ylabel('deg C');
   xlabel(sprintf(' (Year = %d)',yearToday));
   datetick('x', 'mm/dd');
   zoom on;
	grid on;
	colordef black
    ...
    orient portrait
   graph_num = num2str(n);
   d = sprintf('graph_file = ''clim1-%s.jpg'';',graph_num);
   eval(d); 
...
%    'color', 'cmyk','height',4,'width',7);
   ...
   %----------------------------------------------------------
   % multiply by 0.5 to correct magnitude
   max_wspd = 0.5 * c24(ind);
   min_wspd = 0.5 * c25(ind);
   wind_speed = 0.5 * c14(ind);
 ...
 orient portrait   
   graph_num = num2str(n);
   d = sprintf('graph_file = ''clim2-%s.jpg'';',graph_num);
   eval(d); 
...
%    'color', 'cmyk' ,'height',4,'width',7);
 ...
 orient portrait   
   graph_num = num2str(n);
   d = sprintf('graph_file = ''clim3-%s.jpg'';',graph_num);
   eval(d); 
...
%    'color', 'cmyk' ,'height',4,'width',7);
 ...
 orient portrait   
   graph_num = num2str(n);
   d = sprintf('graph_file = ''clim4-%s.jpg'';',graph_num);
   eval(d); 
...
%   d = sprintf('graph_file = %sclim5-%s.jpg%s;',39,graph_num,39);
   d = sprintf('graph_file = ''sclim5-%s.jpg'';',graph_num);
   eval(d); 
...
%   rain_lo = rain_hi - 150.0;
 ...
 orient portrait   
   graph_num = num2str(n);
   d = sprintf('graph_file = ''clim6-%s.jpg'';',graph_num);
   eval(d); 
...
%   rain_hi = ceil(max(cum_rain)/50)*50;
 ...
 orient portrait   
   graph_num = num2str(n);
   d = sprintf('graph_file = ''clim7-%s.jpg'';',graph_num);
   eval(d); 
