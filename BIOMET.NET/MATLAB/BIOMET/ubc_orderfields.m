...
%   $Revision: 1.2 $  $Date: 2002/03/16 13:55:18 $

% 
% See also STRUCT, FIELDNAMES, CELL2STRUCT, STRUCT2CELL.
...
elseif isnumeric(s2)   % ORDERFIELDS(S1, PERM)
    ...
    n = length(newfields);
    if ( length(perm) ~= n )
        error('Permutation vector length must be the same as the number of fields in the struct.');
    end
...
elseif isstruct(s2)   % ORDERFIELDS(S1, S2)
    ...
    if ~all(strcmp(newfields1,newfields2))
        error('Both structures must have exactly the same fields.');
    end
...
elseif iscell(s2)     % ORDERFIELDS(S1, C)
    ...
    if ~all(strcmp(newfields1,newfields2))
        error('The strings of the cell array must match the fieldnames of the struct.');
    end
    % perm1(invperm2) maps original s1 order to original s2 order
    invperm2(perm2) = (1:length(perm2))';
    perm = perm1(invperm2);
    ...
    error('Second argument must be struct, cell array of strings, or a permuation vector.');
end
