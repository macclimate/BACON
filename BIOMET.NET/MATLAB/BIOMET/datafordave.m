...
%                           to all output names
% modified on oct 2006, to include precipitation 2 data and to correct for
% the time shifts. Interception data is now in GMT for the year
...
tv = read_bor([ pth_cl 'clean\clean_tv'],8,[],years);
% tv1 = read_bor([ pth_cl 'clean\clean_tv'],8,[],years);
%Nov 23, 2000
...
P_tot2    = read_bor([pth_cl 'clean\precipitation_2'],[],[],years,indOut);
...
case 'interception'
   if plot_flag
    ...
    eval(['save ' eh_pth  num2str(years) '_Precip1' ' P_tot' ' -ascii;']);
    ...
    eval(['save ' eh_pth  num2str(years) '_year' ' year' ' -ascii;']); 
     eval(['save ' eh_pth  num2str(years) '_Precip2' ' P_tot2' ' -ascii;']);
end
