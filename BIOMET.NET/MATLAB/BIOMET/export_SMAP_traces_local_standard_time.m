...
case 'BS'
   ...
   %GMTOffset = -6/24;
   SubSite_str = 'FlxTwr';
   Rs      = read_bor(fullfile(pth,'climate\clean','radiation_shortwave_downwelling_20m'),[],[],Years);
   Rs = Rs(indOut);
   ppfd    = read_bor(fullfile(pth,'climate\clean','Down_PAR_AbvCnpy_25m_UBC'),[],[],Years);
   ppfd = ppfd(indOut);
case 'PA'
   Site     = 000;
   Site_str = 'OA';
   ...
   SubSite_str = 'FlxTwr';
   %GMTOffset = -6/24;
   Rs      = read_bor(fullfile(pth,'climate\clean','radiation_shortwave_downwelling_36m_UBC'),[],[],Years);
   Rs = Rs(indOut);
   ppfd    = read_bor(fullfile(pth,'climate\clean','Down_PAR_AbvCnpy_37m_UBC'),[],[],Years);
   ppfd = ppfd(indOut);
end

% read in all data

Rn      = read_bor(fullfile(pth,'clean\secondstage','radiation_net_4way_components'),[],[],Years);  
Rn = Rn(indOut);
RnGF    = read_bor(fullfile(pth,'clean\thirdstage','radiation_net_main'),[],[],Years);
RnGF = RnGF(indOut);
RsGF    = read_bor(fullfile(pth,'clean\thirdstage','global_radiation_main'),[],[],Years);
RsGF = RsGF(indOut);
ppfdGF  = read_bor(fullfile(pth,'clean\thirdstage','ppfd_downwelling_main'),[],[],Years);
ppfdGF = ppfdGF(indOut);
tair    = read_bor(fullfile(pth,'clean\secondstage','air_temperature_main'),[],[],Years);
tair = tair(indOut);
tairGF  = read_bor(fullfile(pth,'clean\thirdstage','air_temperature_main'),[],[],Years);
tairGF = tairGF(indOut);
vpd     = read_bor(fullfile(pth,'clean\secondstage','vpd_main'),[],[],Years);
vpd = vpd(indOut);
vpdGF   = read_bor(fullfile(pth,'clean\thirdstage','vpd_main'),[],[],Years);
vpdGF = vpdGF(indOut);
tsoil   = read_bor(fullfile(pth,'clean\secondstage','soil_temperature_main'),[],[],Years);
tsoil = tsoil(indOut);
tsoilGF = read_bor(fullfile(pth,'clean\thirdstage','soil_temperature_main'),[],[],Years);
tsoilGF = tsoilGF(indOut);
prec    = read_bor(fullfile(pth,'clean\thirdstage','precipitation_cumulative'),[],[],Years);
prec  = prec(indOut);
swc   =  read_bor(fullfile(pth,'clean\secondstage','soil_moisture_1'),[],[],Years);
swc   = swc(indOut);
swcGF    =  read_bor(fullfile(pth,'clean\secondstage','soil_moisture_main'),[],[],Years);
swcGF   = swcGF(indOut);
nee     = read_bor(fullfile(pth,'clean\secondstage','nee_ustar_filtered'),[],[],Years);
nee   = nee(indOut);
neeGF   = read_bor(fullfile(pth,'clean\thirdstage','nee_filled_with_fits'),[],[],Years);
neeGF   = neeGF(indOut);
gpp     = read_bor(fullfile(pth,'clean\thirdstage','eco_photosynthesis_measured_smap'),[],[],Years);
gpp     = gpp(indOut);
gppGF   = read_bor(fullfile(pth,'clean\thirdstage','eco_photosynthesis_filled_with_fits'),[],[],Years);
gppGF   = gppGF(indOut);
Reco    = read_bor(fullfile(pth,'clean\secondstage','eco_respiration_measured_ustar_filtered'),[],[],Years);
Reco    = Reco(indOut);
RecoGF  = read_bor(fullfile(pth,'clean\thirdstage','eco_respiration_filled_with_fits'),[],[],Years);
RecoGF  = RecoGF(indOut);
pbar    = read_bor(fullfile(pth,'clean\thirdstage','barometric_pressure_main'),[],[],Years);
pbar    = pbar(indOut);
rdwpot  = read_bor(fullfile(pth,'clean\thirdstage','radiation_downwelling_potential'),[],[],Years);
rdwpot  = rdwpot(indOut);
snowd   = read_bor(fullfile(pth,'clean\secondstage','snow_depth_main'),[],[],Years);
snowd   = snowd(indOut);
