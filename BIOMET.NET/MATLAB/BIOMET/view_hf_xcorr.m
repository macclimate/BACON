function view_hf_xcorr(fig)
%last change: 04.06.2003 nk

% Extract the data
user_data_in = get(fig,'UserData');
currentDate  = user_data_in(1).currentDate;
for i = 1:length(user_data_in)
   data(:,i)         = user_data_in(i).data(1:2^11);
end

% Do correlations
[xc,lag_ind] = ubc_xcorr(detrend(data),'coeff');
lag = lag_ind;

% Assemble user_data for new figure
user_data.data = xc;
user_data.xdata = lag;
user_data.type = 'xcorr';
user_data.titles  = [user_data_in(:).title];
user_data.units  	= [user_data_in(:).unit];
user_data.string1 = [user_data_in(:).string1];
