...
for j=1:length(data_fcrn)
    if j==1
        disp(sprintf('%40s    %30s  %20s','UBC_Variable_Name','DIS_Variable_Name','ini_file_Stage'));
		disp(sprintf('%40s    %30s  %20s','-----------------','-----------------','--------------'));
        fprintf(fid,'%40s    %30s  %20s\n','UBC_Variable_Name','DIS_Variable_Name','ini_file_Stage');
		fprintf(fid,'%40s    %30s  %20s\n','-----------------','-----------------','--------------');
    end
    disp(sprintf('%40s    %30s  %20s',data_fcrn(j).variableName,data_fcrn(j).ini.FCRN_Variable,num2str(data_fcrn(j).inifile)));
