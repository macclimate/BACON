...
%   $Revision: 1.16 $  $Date: 1998/10/20 21:06:39 $
...
if ~ishold,
    if nargin==2,
      ...
      ticks = bestscale(ax,vmin,vmax,dateform,dateChoice);
    else
      [ticks,dateform] = bestscale(ax,vmin,vmax);
    end
else
    ticks = get(gca,[ax,'tick']);
    % Use dateform from bestscale
    [dum,dateform] = bestscale(ax,min(ticks),max(ticks));
end
...
if nargin<5, dateChoice = 'yqmwdHMS'; end
if nargin<4, dateform = []; end
penalty = 0.03;
formlen = [20 12 8 3 1 1 5 1 3 1 4 2 5 8 11 5 8 5 2];
...
htemp = text('units','pixels',...
             'fontname',get(gca,'fontname'), ...
             'fontangle',get(gca,'fontangle'),...
             'fontsize',get(gca,'fontsize'),...
             'fontunits',get(gca,'fontunits'),...
             'fontweight',get(gca,'fontweight'),'visible','on');
if strcmp(ax,'x')
  s = repmat('W',1,length(formlen));
  set(htemp,'string',s)
  ...
  extent = ext(3)/length(formlen)*len/pos(3); 
else
  set(htemp,'string',('WW')')
  ext = get(htemp,'extent');
