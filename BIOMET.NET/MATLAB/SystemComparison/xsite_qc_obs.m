function xsite_qc_obs

%---------------------------------------------------------------------
% Diagnostics
tv_exp = datenum(2004,7,17):floor(now);
...
[tv_ex,h_cpop,varOut,textResults] = fcrn_clean(Stats_xsite,{'XSITE_CP','XSITE_OP'},'report_cpop',tv_exclude_man_cpop,1);
fcrn_print_report(h_cpop,'e:\',textResults);

%---------------------------------------------------------------------
% SITE OP - CP
tv_exp = datenum(2004,7,20):floor(now);
cd D:\Experiments\QC_OBS\Setup
Stats_site = fcrn_load_data(tv_exp);
Stats_site(1).Configuration.pth_tv_exclude = Stats_site(1).Configuration.hhour_path;

tv  = get_stats_field(Stats_site,'TimeVector');
wd  = get_stats_field(Stats_site,'Instrument(5).MiscVariables.WindDirection');
AGC_main  = get_stats_field(Stats_site,'Instrument(end-1).MiscVariables.AGC_Max');
CSAT_diag_max  = get_stats_field(Stats_site,'Instrument(end-3).Max(end)');
tv_exclude_man_main = tv(find(wd<90 | tv<datenum(2004,7,20.5) | AGC_main>65 | CSAT_diag_max > 63));

[tv_ex,h_cpop,varOut,textResults] = fcrn_clean(Stats_site,{'MainEddy','MainEddy_OP'},'report_cpop',tv_exclude_man_main,1,[10 10]);
% fcrn_print_report(h_cpop,'e:\',textResults);
...
fcrn_clean(Stats_all,{'XSITE_CP','MainEddy'},'report_var',[],0,[20 10]);
[tv_exclude,h_rep,varOut,textResults] = fcrn_clean(Stats_all,{'XSITE_CP','MainEddy'},'report_var',tv_exclude_man,1,[20 10]);
