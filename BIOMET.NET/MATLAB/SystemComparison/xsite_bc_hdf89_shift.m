...
sample_shift_licor = NaN .* zeros(size(tv));
t_shift_licor = NaN .* zeros(size(tv));
...
for i = 1:length(tv)
    ...
    try
        ...
        l_yf = fr_read_Digital2(tv(i),c_yf,5);
        ...
        t_shift_licor(i) = sample_shift_licor(i)./20;
...
plot(tv-datenum(2004,1,0),t_shift,tv-datenum(2004,1,0),t_shift_licor)