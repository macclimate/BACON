function [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP] = FCRN_CO2Flux2NEP_Ver1_030812(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTH,FracSEBClosure,LogFile,Plots); 
...
%	First, fit the bR parameters using binned means of all of the data. 
	...
	nReg 					= length(iReg); 
	bRGuess	 			= [10 0.10 10]; 
	bR 					= fr_function_fit(fTs2R,bRGuess,R(iReg),[],Ts(iReg)); 
	RHat0 				= fTs2R(bR,TsGF); 
	...
	for jReg1 = 1:WindowIncR:nReg; 
		...
		if jReg2<=nReg; 
			...
			mtR(i)	 	= mean(t(it)); 
							  ...
							  diary (TempDiaryFile); 
  			mcR(i)	 	= RHat0(it)\R(it); 
 % 			mcR(i)	 	= nanmean(R(it))/nanmean(RHat0(it));
							  diary off; 
...
%	First, estimate the constant bGEP parameters using warm-season data only. 
	...
	bGEPGuess 			= [20 500]; 
%	bGEP 					= nlinfit(PPFD(iReg,:),GEP(iReg),fPPFD2GEP,bGEPGuess); 
	bGEP					= fr_function_fit(fPPFD2GEP,bGEPGuess,GEP(iReg),[],PPFD(iReg,:)); 
...
% 	Use diary to check for nlinfit error messages; skip if nlinfit fails.
...
% 			mcGEP(i)	 	= nanmean(GEP(it))/nanmean(GEPHat0(it));
                              diary off; 
							  fid = fopen(TempDiaryFile,'r'); 
							  cDiary = char(fread(fid))'; 
							  fclose(fid); delete ((TempDiaryFile)); 
...
%	Optional log file.
	...
	if ~isempty(LogFile); 
		...
		fid=fopen(LogFile,'w'); 
		fprintf (fid,'FCRN_CO2Flux2NEP: %s\n',LogFile); 
