...
% * praveena
% August 2014 (Nick)
% Incorporated MPB and HP sites and improved documentation.  Mat used a different approach to
% load second and thirdstage data to pass to the gapfilling code, now
% all of this happens in the site ini files. Tested on legacy years and
% sites and all looks good. All NEP, GEP and R from Mat's/Amanda's/Hughie's thesis and papers 
% obtained using this more generic function. 
% For MPB and HP, thirdstage ustar passed to gapfilling code instead of second stage. This was left for legacy
% reasons. 
%-------------------------------------------------------------------------
...
uStar    = read_db(date_second,SiteId,'clean\secondstage','ustar_main');
...
if ~isempty(date_third)
    try
        ...
        uStar_db         = read_db(date_third,SiteId,'clean\thirdstage','ustar_main');
    end
else
    PPFDGF_db  = [];
    TaGF_db    = [];
...
%-------------------------------------------------------------------------
    ...
    uStar_cur    = evalin('caller','ustar_main');
...
switch upper(SiteId)
    case {'CR','OY','YF'} % VI sites, Kai, Teibo, Christopher Schwalm; log normal empirical model used for R
        [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP] = ...
            FCRN_optimized_for_CR(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTH,mSEBClosure,[],[]); 
    case {'BS','PA','JP','HJP02','HJP75','HJP94'} %BERMS, Iain Hawthorne, Praveena Krishnan; empirical logistic R model
         % note: separate code called for Iain's implementation of wstd
         % despiking-see thridstage ini file for HJP02
        [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP] = ...
            FCRN_CO2Flux2NEP(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTH,mSEBClosure,[],[]); 
    case {'MPB1','MPB2','MPB3'} % MPB Mat Brown, Amanda Mathys (no moving window in winter, soil temperature threshold set at 1 deg C for winter)
        uStar=uStar_tmp(indOut); % Mat used third stage gap-filled ustar--retain for legacy (changing will affect annual NEP,GEP,R)
        [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP] = ...
            FCRN_CO2Flux2NEP_MB_MovWin(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTH,mSEBClosure,[],[]); 
    case {'HP09','HP11'} % Hybrid Poplar, Hughie Jones (no moving window in winter, soil temperature threshold set at 0 deg C for winter) 
        uStar=uStar_tmp(indOut); 
        [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP] = ...
            FCRN_CO2Flux2NEP_HP_MovWin(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTH,mSEBClosure,[],[]); 
    otherwise % use default FCRN gapfilling Barr et al. 2004
        [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP] = ...
            FCRN_CO2Flux2NEP(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTH,mSEBClosure,[],[]); 
end
