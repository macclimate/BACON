function [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP,bR,bGEP,bR_winter] = FCRN_CO2Flux2NEP(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTH,FracSEBClosure,LogFile,Plots); 

%   Revisions
%   %   add parameter to remove NEP when air temp is <0c(due to open-path
%   unreliability during cold conditions)	. 
%FCRN_CO2Flux2NEP
...
%  Aug 27, 2007, interpolation for small gaps were added after applying
%  ustar threshold. % Line 136
%	========================================================================
%	========================================================================

%	Fill small gaps in input data.	
%   Moved NEE gap fill to AFTER R modelling
	GapSizeMax  		= 4; 
...
% 	fPPFD2GEP			= inline(['b(1)*PPFD./(b(2)+PPFD)'],'b','PPFD'); % can also be written in terms of alpha.
	fPPFD2GEP			= inline(['b(2)*PPFD*b(1)./(b(2)*PPFD+b(1))'],'b','PPFD'); %This is how it is written to get the alpha parameter.
...
%	Assign conditions (is*) and indices (i*). 
	...
	isDay 				= ~ isNight; 
	isColdSeason 		= TaGF<=0 & TsGF<=1; %changed to 1C because Ts does not freeze at sites; 
	isWarmSeason 		= TaGF>0 & TsGF>0; 
	isNotColdSeason 	= ~ isColdSeason; 
%   add parameter to remove NEP when air temp is <0c(due to open-path unreliability during cold conditions)	. 	
	isAirCold           = TaGF<=0; 
	...
	iAirCold            = find(isAirCold); 
    isWinter            = TsGF<1;
    [NEP] = FCRN_select_measurements(NEE,uStar,iNight,uStarTH);
    NEP_winter = NEP;
%   remove all cold season NEP measurements 
...
%	First, fit the bR parameters using binned means of all of the data. 
	...
	nReg 					= length(iReg); 
	bRGuess	 			= [20 0.10 20]; 
	[bR,b,c] 					= FCRN_function_fit(fTs2R,bRGuess,R(iReg),[],Ts(iReg)); 
	RHat0 				= fTs2R(bR,TsGF); 
...
%             disp(sprintf('Press any key %d',jReg1));
% figure(100);
% plot(t(it),[RHat0(it) R(it)],'o');
% datetick('x');
% legend('RHat0','R');
% ftitle = sprintf('MovingWindow %d %d %d',WindowWidthR,WindowIncR,i);
% TITLE(ftitle)
% figureFileName = sprintf('D:/Mat/Matlab/Figures/MPB1/MovingWindow_%d_%d_%d',WindowWidthR,WindowIncR,i);
% print('-djpeg',figureFileName)
% pause
...
% calc R in winter
	...
	R_winter 						= NaN*ones(size(NEP_winter)); 
	RHat_winter  					= NaN*ones(size(NEP_winter)); 

%	Assign R = NEP at night and during daytime periods in the cold season. 

	R_winter (iNight)			= -NEP_winter(iNight); 
	R_winter (iColdSeason)		= -NEP_winter(iColdSeason); 
	
%	Fit the R model R = cR(t)*bR(1)/(1+exp[bR(2)*(bR(3)-Ts)]).

%	First, fit the bR parameters using binned means of all of the data. 

	iReg_winter  		 			= find(~isnan(R_winter+Ts)); 
	nReg_winter  					= length(iReg_winter ); 
	bRGuess_winter 	 			= [20 0.10 20]; 
	[bR_winter ,b_winter ,c_winter ] 					= FCRN_function_fit(fTs2R,bRGuess_winter ,R_winter(iReg_winter ),[],Ts(iReg_winter )); 
	RHat0_winter  				= fTs2R(bR_winter ,TsGF); 
...
%	Calculate modeled R (RHat). 
	...
	RHat_winter 					= RHat0_winter; 
%     	RHat 					= cR.*RHat0; 
			
%	Merge R and RHat to produce Rgf (gap-filled). 
				
	Rgf_winter	 				= R_winter; 
	igf_winter 					= find(isnan(R_winter) & ~isnan(RHat_winter)); 
	Rgf_winter(igf_winter) 			= RHat_winter(igf_winter); 
    
%Merge Rgf during the GS and during the winter (no moving window)    
    
    i_win = find(Rgf(isWinter));
    Rgf(i_win)=Rgf_winter(i_win);
%	=======================================================================
%	
...
%	First, estimate the constant bGEP parameters using warm-season data only. 
	...
	bGEPGuess 			= [20 500]; 
	[bGEP,b,c]					= FCRN_function_fit(fPPFD2GEP,bGEPGuess,GEP(iReg),[],PPFD(iReg,:)); 
...
%   Added the following to model GEP during the winter when there was NaNs    
    a = isnan(GEPHat);
	GEPHat(a)=0; 
...
%     NEP	 			  	= FCRN_FillSmallGapsByLinInterp(NEP,GapSizeMax); % Praveena , Aug 27, 2007
	NEPgf 				= NEP; 
