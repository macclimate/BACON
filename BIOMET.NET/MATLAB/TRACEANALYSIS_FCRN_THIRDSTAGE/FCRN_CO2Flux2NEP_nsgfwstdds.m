function [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP] = FCRN_CO2Flux2NEP_nsgfwstdds(t,NEE,wstd,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,wstdTH,FracSEBClosure,LogFile,Plots); 

%this version of the FCRN_CO2Flux2NEP was created by Iain Hawthorne to
%be used additionaly in inifiles for BERMS sites after the original in order to output
%subsequent traces for NEP, GEP and R calculated without gap-filling small 
%gaps in NEP
...
%
%  April 2009, removed gap filling for small gaps, Iain Hawthorne
%	========================================================================
%	========================================================================
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%testscripts
    ...
    dvec          =datevec(t);
    tv            = t-6/24;
    doy           = (tv - datenum(dvec(1,1),1,0));
    
%     figNum = 5;
% figNum  = figNum +1;
%         figure(figNum);
%         whitebg('w');
%         title('No Interpolation, w_{std} filter & despiked');
%     set(gcf,'Color',[0 0 0])
...
%     set(gcf,'Position',max_win,'Color','none','PaperSize',[6 11]);
%     c = [0 0 0;1 0 0;0 1 0;0 0 1; 0.4 0.4 0.4; 0.4 0 0;0 0.4 0;0 0 0.4];
%     hold on;
%     plot(doy,NEE,'x','color',c(2,:),'LineWidth',2,'MarkerSize',12);
%     axis([215.5 216.5 -20 40]);
%     AxisBoxLineWidth = 2;
%     set(gca,'box','on','linewidth',AxisBoxLineWidth)
...
%	Fill small gaps in input data.	
	...
	%NEE	 			  	=
	%FCRN_FillSmallGapsByLinInterp(NEE,GapSizeMax);Iain Hawthorne April
	%2009 should not gap-fill before selecting data to model with.
	wstd				= FCRN_FillSmallGapsByLinInterp(wstd,GapSizeMax); 
...
%	Assign conditions (is*) and indices (i*). 
    ...
    [NEP] = FCRN_select_measurements(NEE,wstd,iNight,wstdTH);
    
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%testscripts
%     hold on; plot(doy,-NEP,'s','color',c(3,:),'LineWidth',2,'MarkerSize',12);
%     legend('NEE raw','NEE w_{std} filtered',2);
%     legend('boxoff');
%     pause;
% 	
%	Apply SEB-closure adjustment to the data.
	...
	%NEP	 			  	=
	%FCRN_FillSmallGapsByLinInterp(NEP,GapSizeMax); % added  by praveena.
	%Removed by Iain Hawthorne April 2009
	%data gaps are often at points when insufficient turbulent mixing in
	%the surface layer or ecosystem flushes. Interpolation is not wise
	%under such circumstances.
    %========================================================================
    GapMax = 13*48;
    
     [NEP,NEPold]=fr_fill_and_despike(NEP,GapMax);
%     
% %     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%testscripts
%      hold on; plot(doy,-NEP,'v','color',c(4,:),'LineWidth',2,'MarkerSize',12);
%      legend('NEE raw','NEE w_{std} filtered','NEE w_{std} filtered & despiked',2);
%      legend('boxoff');
%      pause;
...
%	First, fit the bR parameters using binned means of all of the data. 
	...
	nReg 					= length(iReg); 
	bRGuess	 			= [20 0.10 20]; 
	[bR,FVAL,OUTPUT] 					= FCRN_function_fit(fTs2R,bRGuess,R(iReg),[],Ts(iReg)); 
	RHat0 				= fTs2R(bR,TsGF); 
    bR
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%testscripts
%     text(215.6,-10,['bR', sprintf(' = %4.2f;',bR)]);
%     text(215.6,-15,['{\itr}^{2}', sprintf(' = %4.2f;',OUTPUT.R2)]);
%     pause;
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%testscripte
...
%	Assign final gap-filled NEP = GEP-R values and fill gaps with modeled estimates.	
    ...
    c=12*30*60/1e6; % umol/m2/s to gC m-2; 
		sNEP=c*FCRN_nansum(NEPgf)
        sR=c*FCRN_nansum(Rgf)
        sGEP=c*FCRN_nansum(GEPgf) 
        
        
%     text(216.25,-10,['NEP', sprintf(' = %4.f;',sNEP)]);
%     text(216.25,-13,['R', sprintf(' = %4.f;',sR)]);
%     text(216.25,-16,['GEP', sprintf(' = %4.f;',sGEP)]);
%     pause;
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%testscripte
