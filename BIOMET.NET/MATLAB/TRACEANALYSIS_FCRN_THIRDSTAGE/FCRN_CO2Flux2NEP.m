...
%  July 7, 2014 (Nick)
%       -hard-coded the name for the tmp file used to store regression
%       output--using tempname produced filenames so long they could not be
%       deleted! Just deleted over 6,000 files from the Temp folder on C:\ 
	...
	for jReg1 = 1:WindowIncR:nReg; 
		...
		if jReg2<=nReg; 
			...
			mtR(i)	 	= mean(t(it)); 
                              ...
                              dv=datevec(now);
                              dvstr=[num2str(dv(1)) num2str(dv(2)) num2str(dv(3)) num2str(dv(4)) num2str(dv(5)) num2str(floor(dv(6)))];
                              TempDiaryFile=fullfile(tempdir,['FCRN_' dvstr '.tmp']);
							  diary(TempDiaryFile); 
  			mcR(i)	 	= RHat0(it)\R(it); 
							  diary off; 
