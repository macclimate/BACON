function pl_nep_gep_r_for_web_display(SiteId,Years,doy_data,bwfig);
...
if WHAT == 0 % if cumulative data is being plotted, then also present monthly bar graphs
    ...
    plot(DOY,NEP_FIG(:,end),'LineWidth',3); hold on;
    %axis([1 365 -Inf +Inf]);
    %xlabel('DOY');
    ylabel('g C m^{-2}');
    HLEG = legend(iOut,'location','Best');
    ...
    doy=DOY';
    set(hb1,'Ylim',[min(min(floor(NEP_FIG'))) max(max(ceil(NEP_FIG')))]);
