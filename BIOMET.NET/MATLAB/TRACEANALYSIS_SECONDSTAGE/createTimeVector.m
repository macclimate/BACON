function trace_out = createTimeVector(trace_in) 
...
%case {'cl','fl','ch','pr'}  
   ...
   chgfile = find(fn=='.');
   % Take care of new data logger naming convention
   if isempty(chgfile)
       chgfile = find(fn=='\');
       if length(chgfile) == 1
            chgfile = [0 chgfile];
       end
       fn      = [fn(1:chgfile(end)) fn(chgfile(end-1)+1:chgfile(end)-1) '_tv'];	
   else
...
if exist(fullfile(pth,'TimeVector'))
    timeVector = read_bor(fullfile(pth,'TimeVector'),8);
else
...
trace_in.DOY = timeVector(indOut) - datenum(Year,1,0);
trace_in.DOY = trace_in.DOY - time_shift/24;

% But the time vector should also be only the time in question
trace_in.timeVector = timeVector(indOut);
