...
line([tick_val(4) tick_val(4)+0.5],[900 900],'Color','r','linewidth',2)
text(tick_val(4)+0.6,900,'Sensible Heat','Fontsize',6);
line([tick_val(6) tick_val(6)+0.5],[900 900],'Color','b','linewidth',2)
text(tick_val(6)+0.6,900,'Latent Heat','Fontsize',6);
...
% plot cumulative NEP
pl_single_panel(DOY,NEP_FIG,NEP_MONTHLY,'TSER',legstr,0,'g C m^{-2}',[],2,4,'NEP_{csum}',[.75 .9],10,8,10);
print( '-djpeg',[fullfile(pth_out,lower(['graph_NEP_sum_' SiteId]))]);
% plot monthly NEP
pl_single_panel(DOY,NEP_FIG,NEP_MONTHLY,'MTH',legstr,0,'g C m^{-2}',[],2,4,'NEP_{mon}',[.75 .1],10,8,10);
print( '-djpeg',[fullfile(pth_out,lower(['graph_NEP_mon_' SiteId]))]);
% plot monthly GEP
ylim_gep = pl_single_panel(DOY,GEP_FIG,GEP_MONTHLY,'MTH',legstr,0,'g C m^{-2}',[],2,4,'GEP_{mon}',[.75 .9],10,8,10);
print( '-djpeg',[fullfile(pth_out,lower(['graph_GEP_mon_' SiteId]))]);
% plot monthly R
pl_single_panel(DOY,R_FIG,R_MONTHLY,'MTH',legstr,1,'g C m^{-2}',ylim_gep,2,4,'\itR\rm_{mon}',[.75 .9],10,8,10);
print( '-djpeg',[fullfile(pth_out,lower(['graph_R_mon_' SiteId]))]);

close all;

function [ylim_out] = pl_single_panel(DOY,DATA_FIG,DATA_MONTHLY,figtyp,legstr,xlabflg,ylabstr,ylimover,ht,wd,figlabstr,figlabposn,fsz_axlab,fsz_tcklab,fsz_figlab);
...
switch upper(figtyp)
    ...
    case {'TSER'}
        hb=plot(DOY,DATA_FIG,'LineWidth',2); 
        ylabel(ylabstr,'FontSize',fsz_axlab);
        if ~isempty(legstr)
           HLEG = legend(legstr,'location','SouthWest');
