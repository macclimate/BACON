function fr_bcfn_report(TRACE,TYPE,Y_LABEL,PERIOD,NAN,NONEG,PATH);
% Data visualization tool for the BC FluxNet sites. Generates one graph for each
...
%
% Example: fr_bcfn_report('eco_photosynthesis_main',0,'Cumulative GEP',0,0,1);
%          fr_bcfn_report('precipitation',4,'Cumulative precipitation (mm)');
%          fr_bcfn_report('nep_measured',0,'Daytime cumulative NEP',2);
%          fr_bcfn_report('eco_respiration_measured',5,'14-day mean E_R');
%          fr_bcfn_report('air_temperature_main',8,'14-day mean daytime T_{air}',2);
%
% (c) Christopher R Schwalm Nov 25, 2004
...
for ii = 1:3;
    ...
    %site specifics; ID and yr of establishment --which is not graphed
    switch ii
        case 1
            Years = 1997:current;
            SiteId = 'CR';
    ...
    %set up legend
    dv = datevec(TV_MAT(1,2:length(Years)));
    iWhen = cellstr(num2str(dv(:,1))); 
    %set up cumsum'd totals to display on figure
    if TYPE < 5 
        iDub = cell(length(Years)-1,1);
        for nn = 1:length(Years)-1; iDub{nn} = '       ';  end %just a spacer between the yr and total
        for nn = 1:length(Years)-1;
            ind_last = find(isnan(DATA_FIG(:,nn+1)));
            try
            ...
            try
                iTot(nn) = DATA_FIG(ind_last,nn+1);
            catch
                iTot(nn) = NaN;
            end
        end
    ...
    %draw figure
    figure('Name',figName);
    plot(DOY,DATA_FIG(:,2:length(Years)),'LineWidth',1.5);
    axis([1 365 -Inf +Inf]);
    xlabel('DOY');
    ylabel([Y_LABEL ' ' Y_UNITS]);
    title([SiteId ' from ' num2str(Years(2)) ' to ' num2str(Years(end))]);
    hl = legend(iOut,2);
