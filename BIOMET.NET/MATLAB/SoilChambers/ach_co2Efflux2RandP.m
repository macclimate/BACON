function [Efflux,R,P,Effluxgf,Rgf,Pgf,RHat,PHat,RHat0,PHat0] = ach_co2Efflux2RandP(t,Efflux,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,LogFile,Plots,siteId,chNum,use_mvwin); 

% revisions:

% Feb 6, 2008
%   -revised so that the use of a moving window to fit a seasonal model
%   parameter is optional; new parameter "use_mvwin" gives user control
%   (Nick)
...
%	Specify the (inline) R and P models.
	fTs2R 				= inline(['b(1).*b(2).^((Ts-10)./10)'],'b','Ts'); %Q10 model
...
%   Assign R = Efflux during day for PA and YF (assuming no photosynthesis) 
	if siteId == 'PA'
        R(iDay)			= Efflux(iDay); 
    elseif siteId == 'YF'
        R(iDay)			= Efflux(iDay); 
...
%	First, fit the bR parameters using binned means of all of the data. 
	iReg 		 		= find(~isnan(R+Ts) & R>=0); % only night-time and cold-season data exists in R at this point, all else is NaNs
	nReg 				= length(iReg); 
	bRGuess	 			= [5 2]; 
	bR 					= ach_function_fit(fTs2R,bRGuess,R(iReg),[],Ts(iReg)); 
	RHat0 				= fTs2R(bR,TsGF); 
...
if use_mvwin % use the moving window approach to capture seasonality
    ...
    for jReg1 = 1:WindowIncR:nReg; 
        ...
        if jReg2<=nReg; 
            ...
            RLin = log10(R(it));
            [p,R2Linearfit,sigma,s,Y_hat] = polyfit1(Ts(it),RLin,1);
            bRShort1(i) = p(1);	
            bRShort2(i) = p(2);	
            bRShortR2(i) = R2Linearfit;	
            bRShortRes(i) = sigma;
            bRShortDeltaTs(i) = max(Ts(it))-min(Ts(it));
            bRShortMeanTs(i) = mean(Ts(it));
            
        end; 
    end; 
    
    %     save(['D:\david\david_data\junk\bRShort1_' upper(siteId) num2str(chNum)],'bRShort1');
    %     save(['D:\david\david_data\junk\bRShort2_' upper(siteId) num2str(chNum)],'bRShort2');
    %     save(['D:\david\david_data\junk\bRShortR2_' upper(siteId) num2str(chNum)],'bRShortR2');
    %     save(['D:\david\david_data\junk\bRShortRes_' upper(siteId) num2str(chNum)],'bRShortRes');
    %     save(['D:\david\david_data\junk\bRShortDeltaTs_' upper(siteId) num2str(chNum)],'bRShortDeltaTs');
    
    %remove bad short term parameters (except for ch 2 at YF...)
    if upper(siteId) == 'YF' & chNum == 2
        bRShort1 = bRShort1;
        bRShort2 = bRShort2;
    else
        indNegSlopes = find(bRShort1<0);
        bRShort1(indNegSlopes) = NaN; 
        bRShort2(indNegSlopes) = NaN; 
        
        indBigSlopes = find(bRShort1>1);
        bRShort1(indBigSlopes) = NaN; 
        bRShort2(indBigSlopes) = NaN; 
    end
    
    Q10 = exp(10.*bRShort1);
    R10 = Q10.*exp(bRShort2);
    
    indBadQ10 = find(Q10 > bR(2)+2);
    Q10(indBadQ10) = NaN;
    R10(indBadQ10) = NaN;
    indBadQ102 = find(Q10 > mean(Q10(~isnan(Q10)))+1);
    Q10(indBadQ102) = mean(Q10(~isnan(Q10)));
    R10(indBadQ102) = mean(R10(~isnan(R10)));
    
    bRShort1(indBadQ10) = NaN;
    bRShort2(indBadQ10) = NaN;
    bRShort1(indBadQ102) = NaN;
    bRShort2(indBadQ102) = NaN;
    bRShort1 = FCRN_naninterp1(mtR,bRShort1,t,'linear'); 
    bRShort2 = FCRN_naninterp1(mtR,bRShort2,t,'linear'); 
    
    Q10 = FCRN_naninterp1(mtR,Q10,t,'linear'); 
    R10 = FCRN_naninterp1(mtR,R10,t,'linear'); 
    
    %     save(['D:\david\david_data\junk\bRShort1GF_' upper(siteId) num2str(chNum)],'bRShort1');
    %     save(['D:\david\david_data\junk\bRShort2GF_' upper(siteId) num2str(chNum)],'bRShort2');
    ...
    bRShort = [bRShort1 bRShort2];
    for j = 1:length(TsGF)
        RHat2(j) = polyval(bRShort(j,:),TsGF(j));
    end
    
    
    RHat1 = cR.*RHat0;
    RHat2 = 10.^(RHat2); %inverse log10
    
    %	Merge R and RHat to produce Rgf (gap-filled). 
    Rgf	 			= R; 
    %if use_mvwin % use the moving window approach to capture seasonality
    igf 			= find(isnan(R) & ~isnan(RHat2')); 
    Rgf(igf) 		= RHat2(igf); 
    
else % just use the annual relationship
    Rgf	 			= R;
    igf 			= find(isnan(R) & ~isnan(RHat0)); 
    Rgf(igf) 		= RHat0(igf);
...
%	Set P to zero during day for PA and YF (assuming no photosynthesis). 
	if siteId == 'PA'
		P(iDay) 	= 0; 
    elseif siteId == 'YF'
        P(iDay)	    = 0; 
...
%	First, estimate the constant bP parameters using warm-season data only.
	...
	bPGuess 			= [20 500]; 
	bP					= ach_function_fit(fPPFD2P,bPGuess,P(iReg),[],PPFD(iReg,:)); 
...
%	4. R vs Ts. 
	...
	if ismember(4,Plots); figure(4); clf; % (Ts,R)
		...
		grid on; hold off; 
		xlabel('Ts'); ylabel('R'); 
		legend('Measured','Estimated','Annual',2); 
	end; 	
