function data_cleaned = ach_cleaning_siteyear(Year,SiteId,stage,db_ini,cr_acs_flag)
...
% Revisions:
% Dec 14, 2009
%   -Revised to handle either of the CR chamber systems (ACS or single
%   chamber) (Nick)
...
% Nick added mods to handle CR single ch and ACS system (Dec 14, 2009)
if upper(SiteId) == 'CR' & cr_acs_flag
    ini_file_first  = fullfile(pth_proc,[SiteId '_Chambers_FirstStage_ACS-DC.ini']);
elseif upper(SiteId) == 'CR' & ~cr_acs_flag
    ini_file_first  = fullfile(pth_proc,[SiteId '_Chambers.ini']);
end

% Nick added mods to handle CR single ch and ACS system (Dec 14, 2009)
if upper(SiteId) == 'CR' & cr_acs_flag
   ini_file_second = fullfile(pth_proc,[SiteId '_Chambers_SecondStage_ACS.ini']);
   ini_file_third  = fullfile(pth_proc,[SiteId '_Chambers_ThirdStage_ACS.ini']);
else
...
if stage == 1
    ...
    % Nick added mods to handle CR single ch and ACS system (Dec 14, 2009)
    if upper(SiteId) == 'CR' & cr_acs_flag
          mat_file = fullfile(pth_proc,[SiteId '_' num2str(yy_str) '_Chambers_ACS.mat']);
    elseif upper(SiteId) == 'CR' & ~cr_acs_flag
          mat_file = fullfile(pth_proc,[SiteId '_' num2str(yy_str) '_Chambers.mat']);
    end
