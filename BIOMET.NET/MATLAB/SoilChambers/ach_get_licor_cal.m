...
%(c) dgg
%Created:     Nov 26, 2003
%Revisions:   Feb 1, 2006
%
% - Feb 1, 2006 (dgg)
%   - changed function that loads data : from ach_read_data to
%   ach_find_and_read_data, this avoids loading a different day than requested
%   by the program
% - Aug 20, 2005
%   - corrected bug: ~exist(SiteID) changed to ~exist(SiteFlag) to match
%   the input paramter (Z)
% - Dec 05, 2003:
...
for i = 1:length(currentDate_vec)
   ...
   try
      [data_HF,data_HH] = ach_find_and_read_data(currentDate,SiteFlag,c.chamber.systemType);
