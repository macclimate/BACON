...
if siteID == 'PA';
   ...
   chNbr = 8;
elseif siteID == 'BS' 
   traceName_ev = 'stats.Chambers.Fluxes_Stats.evOutLong.ev(:,[5:10])';
%   traceName_ev = 'stats.Chambers.Fluxes_Stats.evOutLong.ev(:,[1:12])';
   traceName_fluxes = 'stats.Chambers.Fluxes_Stats.fluxOutLong.flux(:,[5:10])';
...
%   legendTmp = ['ch01';'ch02';'ch03';'ch04';'ch05';'ch06';'ch07';'ch08';'ch09';'ch10';'ch11';'ch12'];
   legendTmp = ['ch05';'ch06';'ch07';'ch08';'ch09';'ch10'];
   chNbr = 6;
%   chNbr = 12;
elseif siteID == 'JP'
...
elseif siteID == 'YF'
   ...
   %legendTmp = ['ch01';'ch02';'ch03';'ch04';'ch05';'ch06';'ch07';'ch08';'ch09';'ch10';'ch11';'ch12'];
   %chNbr = 12;
   traceName_ev = 'stats.Chambers.Fluxes_Stats.evOutLong.ev(:,[1:7])';
   traceName_fluxes = 'stats.Chambers.Fluxes_Stats.fluxOutLong.flux(:,[1:7])';
   legendTmp = ['ch01';'ch02';'ch03';'ch04';'ch05';'ch06';'ch07'];
   chNbr = 7;
end
...
%root exlcusion for SOBS
if siteID == 'BS'
try
fig = fig+1;figure(fig);clf
traceName = traceName_fluxes;
[DecDOY,x] = ach_join_trace(traceName,st+yearOffset,ed+yearOffset,fileExt,pth);
plot(t,x(ind,3),'r-.'); %ch8
hold on;
plot(t,x(ind,5),'b-.'); %ch9
plot(t,x(ind,3),'r-','LineWidth',1.5); %ch7
plot(t,x(ind,6),'b-','LineWidth',1.5); %ch10
% hold on;
% plot(t,x(ind,8),'b-.');
% plot(t,x(ind,9),'g-.');
% plot(t,x(ind,12),'c-.');
% plot(t,x(ind,4),'r-','LineWidth',1.5);
% plot(t,x(ind,7),'b-','LineWidth',1.5);
% plot(t,x(ind,10),'g-','LineWidth',1.5);
% plot(t,x(ind,11),'c-','LineWidth',1.5);
hold off;
%legend('ch3','ch8','ch9','ch12','ch4','ch7','ch10','ch11');
legend('ch8','ch9','ch7','ch10');
axis([t(1) t(end) -1 10]);
grid on;zoom on;
xlabel('DOY');
...
traceName = traceName_fluxes;
[DecDOY,x] = ach_join_trace(traceName,st+yearOffset,ed+yearOffset,fileExt,pth);
%Rhr = (x(ind,4) + x(ind,7) + x(ind,10) + x(ind,11)) ./4 ;  %control plots
%Rh = (x(ind,3) + x(ind,8) + x(ind,9) + x(ind,12)) ./4 ;   %root excluded plots
