...
%                       Last modification:  Feb 19, 2013

%
% Revisions:
% Feb 19, 2013: Nick corrected an indexing problem affecting the plot comparing ini
%              file cal ppm to cal file ppm; corrected handling of ignored
%              calibrations
...
%     cal_values = cal_values(ind_notignore);     % Feb
end
%--------------------------------------------------------------------------
...
indc=[];
for i = 1:length(ind)
    if ismember(ind(i),ind_notignore) % Feb 19, 2013: indexing of c has to be stored for plotting ini file values
        indc=[indc; i];
    end
    %c(i)= fr_get_init(SiteID,TimeVector(i));                 % get the most current init file 
    
    c(i)= fr_get_init(SiteID,tv(ind(i)));  
...
ind=intersect(ind,ind_notignore);
...
plot(decDOY(ind),[CAL_1_cal(ind),CAL_2_cal(ind),Li_No_cal(ind)],'h',...
    decDOY(ind),[CAL_1_ini(indc),CAL_2_ini(indc),Li_No_ini(indc)],'o-'); % Feb 19, 2013
