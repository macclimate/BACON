function [EngUnits,Header,tv] = fr_read_TOA5_file(fileName,time_str_flag,headerlines,defaultNaN,assign_in)
...
s_read = textread(fileName,'%q',1,'headerlines',headerlines);
N = length(split_line(char(s_read)));

% Read header
if headerlines > 0
    for i = 1:headerlines
        s_read = textread(fileName,'%q',1,'headerlines',i-1);
...
Header.loggerSN = str2double(char(Header.line1(2)));
