function [numOfFilesProcessed,numOfDataPointsProcessed] = fr_site_met_database(wildCardPath,chanInd,chanNames,tableID,processProgressListPath,databasePath,fileType,time_shift)
    ...
    if filesProcessProgressList(j).Modified < datenum(h(i).date)
        try
            ...
            switch fileType
                ...
                case 2
                    [climateData,Header,tv] = fcrn_read_csi_ascii(fullfile(pth,h(i).name));
            ...
            if ~isempty(findstr(databasePath,'\yyyy\'))
                ...
                for year_ind = years
                    ...
                    [k] = db_new_eddy(ClimateStats(one_year_ind),[],databasePathNew,0);
                end
            else
                [k] = db_new_eddy(ClimateStats,[],databasePath,0);
            end
