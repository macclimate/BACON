...
%
%   Nov 14, 2017 (Zoran)
%       - added extrapoloation option so the function does not return NaN-s
%       if the target tv is outside of the tv range. Done for
%       Resampling_type == 1 (ONLY!)
%       This:
%           interp1(tv_seconds,Instrument_data(currentInstrument).EngUnits, tv_seconds_target);
%       was changed to:
%           interp1(tv_seconds,Instrument_data(currentInstrument).EngUnits, tv_seconds_target,'linear','extrap');
%   Oct 18, 2017 (Zoran)
%       - Major revisions
