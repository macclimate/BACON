function cal_pl_li7000(indX,Year,SiteID,pth)
% cal_pl_li7000(indX,Year,SiteID,pth) Plot LI-7000 calibrations
% 
% 
% (c) Zoran Nesic       File created:       Dec 15, 2004
%                       Last modification:  Aug 27, 2013

%
% Revisions:
% Aug 27, 2013 (Z)
%   - Added figure names
% Mar 23, 2010: applied GMToffset to tv (Nick)
...
%------ Mar 23/2010: apply GMToffset to tv -----
tv=tv+GMToffset;
%-----------------------------------------------
...
for i = 1:length(ind)
    c(i)= fr_get_init(SiteID,tv(end));                 % get the most current init file 
    ...
    co2_hf(:,i) = cal_values(ind(i)).HF_data(:,1);
    h2o_hf(:,i) = cal_values(ind(i)).HF_data(:,2);
    Tb_hf(:,i)  = cal_values(ind(i)).HF_data(:,3);
    Pb_hf(:,i)  = cal_values(ind(i)).HF_data(:,4);    
    Pg_hf(:,i)  = cal_values(ind(i)).HF_data(:,5);    
...
kk=length(co2_hf);
if size(co2_hf,2) < 7
    for i=1:size(co2_hf,2)
        co2_hf(kk-(i-1)*10:end,i) = NaN; %#ok<AGROW>
    end
end
plot(tv_hf,co2_hf,'.-')
...
kk=length(h2o_hf);
if size(h2o_hf,2) < 7
    for i=1:size(h2o_hf,2)
        h2o_hf(kk-(i-1)*10:end,i) = NaN; %#ok<AGROW>
    end
end
plot(tv_hf,h2o_hf,'.-')
...
kk=length(Tb_hf);
if size(Tb_hf,2) < 7
    for i=1:size(Tb_hf,2)
        Tb_hf(kk-(i-1)*10:end,i) = NaN; %#ok<AGROW>
    end
end
plot(tv_hf,Tb_hf,'.-')
title('Time response T_{LICOR}')
set_figure_name(SiteID)
xlabel('t (s)')
ylabel('T_{LICOR} (degC)')
zoom on
...
kk=length(Pb_hf);
if size(Pb_hf,2) < 7
    for i=1:size(Pb_hf,2)
        Pb_hf(kk-(i-1)*10:end,i) = NaN; %#ok<AGROW>
    end
end
plot(tv_hf,Pb_hf,'.-')
title('Time response P_{LICOR}')
set_figure_name(SiteID)
xlabel('t (s)')
ylabel('P_{LICOR} (kPa)')
zoom on
...
kk=length(Pg_hf);
if size(Pg_hf,2) < 7
    for i=1:size(Pg_hf,2)
        Pg_hf(kk-(i-1)*10:end,i) = NaN; %#ok<AGROW>
    end
end
plot(tv_hf,Pg_hf,'.-')
title('Time response P_{gauge}')
set_figure_name(SiteID)
xlabel('t (s)')
ylabel('P_{gauge} (mV)')
zoom on
...
function set_figure_name(SiteID)
     ...
     set(gcf,'Name',[ SiteID ': ' char(title_string)],'number','off')