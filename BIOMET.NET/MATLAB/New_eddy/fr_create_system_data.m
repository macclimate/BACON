...
%   - LGR_CH4N2O is assumed to be Instrument #3!!
%   - Changed 
%       for i=1:2
%      to
%       for i = 1:length(configIn.System(systemNum).Delays.ArrayLengths)
% May 11, 2012
%   -added pre-processing hook to execute preprocessing code from init_all
%   if desired
% July 18, 2011
...
% STEP - Run preprocessing-added May 11, 2012
if isfield(configIn.System(systemNum),'PreProcessData') ...
          && ~isempty(configIn.System(systemNum).PreProcessData)
    for j=1:length(configIn.System(systemNum).PreProcessData)
        eval(char(configIn.System(systemNum).PreProcessData(j)));
    end
end
...
for currentInstrument = configIn.System(systemNum).Instrument              % cycle for each instrument in the system
    ...
    if ~isempty(Instrument_data(currentInstrument).EngUnits)
        EngUnits_alignment_only = [EngUnits_alignment_only Instrument_data(currentInstrument).EngUnits(:,configIn.Instrument(currentInstrument).CovChans)];     %#ok<*AGROW> % append each individual instrument
    else
