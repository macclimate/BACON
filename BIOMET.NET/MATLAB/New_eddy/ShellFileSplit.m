...
% Revisions
% Dec 4, 2012 (Nick)
%   -placed final save of datafiles before exiting loop inside
%   try-catches--in the event no datalines of a particular instrument type
%   are found this prevents crashing the program
% May 9, 2012 (Zoran)
...
%EC Logger station parameters
maxLinesEC = 3;
oldTempTVstringEC = [];
currentTimeStringEC = [];
fileNameExtensionEC = '.dSQT5';
colHeaderEC = {'RECORD','BattVolt_Avg','LoggerRefTemp_Avg','TC_Temp1_Avg',...
               'TC_Temp2_Avg','Echo_mV1_Avg','Echo_mV2_Avg','BattVolt_Min',...
               'LoggerRefTemp_Min','TC_Temp1_Min','TC_Temp2_Min','Echo_mV1_Min',...
               'Echo_mV2_Min','BattVolt_Max','LoggerRefTemp_Max','TC_Temp1_Max',...
               'TC_Temp2_Max','Echo_mV1_Max','Echo_mV2_Max','BattVolt_Std',...
               'LoggerRefTemp_Std','TC_Temp1_Std','TC_Temp2_Std','Echo_mV1_Std',...
               'Echo_mV2_Std'};
numOfChannelsEC = length(colHeaderEC);
...
while 1, 
    ...
    if ~ischar(tline) || k > kmax,
        ...
        try
          save_data(fileNameIRGA,irgaLine,dataOutIRGA,colHeaderIRGA)
            ...
            disp(sprintf('Error saving IRGA data or no IRGA data found in %s',dailyFileName));
        end
        try
           save_data(fileNameSonic,sonicLine,dataOutSonic,colHeaderSonic)
        catch
           disp(sprintf('Error saving sonic data or no sonic data found in %s',dailyFileName));
        end
        try
           save_data(fileNameWS,WSLine,dataOutWS,colHeaderWS)
        catch
            disp(sprintf('Error saving climate data or no climate data found in %s',dailyFileName)); 
        end
        try
            save_data(fileNameGF,GFLine,dataOutGF,colHeaderGF)
        catch
            disp(sprintf('Error saving GasFinder data or no GasFinder data found in %s',dailyFileName)); 
        end
        try
           save_data(fileNameEC,ECLine,dataOutEC,colHeaderEC)
        catch
           disp(sprintf('Error saving EC logger data or no EC logger data found in %s',dailyFileName));  
        end
    ...
    elseif strfind(tline,'$ECDTA,') > 0
        %this is a EC logger station line.  Extract time from it:
        ind=find(tline == ',');
        tempTVstringEC = tline(ind(1)+[1:19]);        
        % to reduce the number of time datenum is called
        % first test if the time string has changed at all   
        if isempty(oldTempTVstringEC) || ~strcmp(oldTempTVstringEC(15:16),tempTVstringEC(15:16))  
            oldTempTVstringEC = tempTVstringEC;
            tv = datenum(tempTVstringEC);          % the original data does not have miliseconds.  Add one second to time to put data in the correct hhour
            newTimeStringEC = datestr(fr_round_time(tv+1/24/60/60,'30min',2),30);
        end
        if isempty(currentTimeStringEC) 
            % first file creation.  Set all variables
            fileNameEC = make_file_name(dataPath,newTimeStringEC,fileNameExtensionEC);
            dataOutEC = zeros(maxLinesEC,numOfChannelsEC);
            ECLine = 0;   % reset the line counter
            currentTimeStringEC = newTimeStringEC;   % set the currentTimeStringIRGA
        end    
        if ~strcmp(currentTimeStringEC, newTimeStringEC)
            % time to close the old file and open the new one (end of
            % half-hour)
        ...
        try
            ECLine = ECLine+1;
            formatLine = [];
            for cc=1:numOfChannelsEC
                formatLine = [formatLine '%f']; %#ok<AGROW>
            end            
            c = textscan(tline(ind(2)+1:end-1),formatLine,'delimiter',',');
            for i = 1:length(c)
                if isempty(c{i})
                    c{i} = NaN;
                end
            end        
            [RECORD,BattVolt_Avg,LoggerRefTemp_Avg,TC_Temp1_Avg,...
               TC_Temp2_Avg,Echo_mV1_Avg,Echo_mV2_Avg,BattVolt_Min,...
               LoggerRefTemp_Min,TC_Temp1_Min,TC_Temp2_Min,Echo_mV1_Min,...
               Echo_mV2_Min,BattVolt_Max,LoggerRefTemp_Max,TC_Temp1_Max,...
               TC_Temp2_Max,Echo_mV1_Max,Echo_mV2_Max,BattVolt_Std,...
               LoggerRefTemp_Std,TC_Temp1_Std,TC_Temp2_Std,Echo_mV1_Std,...
               Echo_mV2_Std]=deal(c{:});
            dataOutEC(ECLine,1:numOfChannelsEC) = [RECORD,BattVolt_Avg,LoggerRefTemp_Avg,TC_Temp1_Avg,...
               TC_Temp2_Avg,Echo_mV1_Avg,Echo_mV2_Avg,BattVolt_Min,...
               LoggerRefTemp_Min,TC_Temp1_Min,TC_Temp2_Min,Echo_mV1_Min,...
               Echo_mV2_Min,BattVolt_Max,LoggerRefTemp_Max,TC_Temp1_Max,...
               TC_Temp2_Max,Echo_mV1_Max,Echo_mV2_Max,BattVolt_Std,...
    ...
    elseif isempty(strfind(tline,'$GFDBG,'))
        disp(sprintf('** Not able to process the line: %s',tline))
    end    
end;
fclose('all');
