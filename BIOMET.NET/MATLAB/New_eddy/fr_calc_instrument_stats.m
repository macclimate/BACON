...
%                           Last modification:  Nov  6, 2017

% Revisions
% Nov 6, 2017 (Z)
%   - Found a bug: the wind directions were not corrected for orientation 
%     when doing the histogram calculations (only for the MiscVariables.WindDirection)
%     Chaged it from:
%               dir = FR_Sonic_wind_direction([Instrument_data(i).EngUnits(:,1:3)]',InstrumentStats(i).Type);
%             to:
%               dir = FR_Sonic_wind_direction([Instrument_data(i).EngUnits(:,1:3)]',InstrumentStats(i).Type);
%               dir = mod(dir+Orientation+360,360);
% Jan 28, 2008 (Z)
    ...
    switch upper(InstrumentStats(i).Type)
        ...
        case {'7200'}
            try
                ind_diag = find(strcmp(lower(configIn.Instrument(i).ChanNames),'diag'));
                if ~isempty(ind_diag)
                    InstrumentStats(i).MiscVariables = fr_get_LI7200_diagnostics(Instrument_data(i).EngUnits(:,ind_diag(1)));        
                end
            end
            
        case {'CSAT3','GILLR2','GILLR3' 'RMY'}
            try %added this Mar 31,2003 - deals with halfhours where sensor info is missing
                ...
                dir = FR_Sonic_wind_direction([Instrument_data(i).EngUnits(:,1:3)]',InstrumentStats(i).Type);
                % correct the angles for histogram too (Zoran 20171106)
                dir = mod(dir+Orientation+360,360);
