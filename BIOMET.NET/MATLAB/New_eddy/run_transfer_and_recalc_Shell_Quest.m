...
% Revisions
% Dec 4, 2012
%   -recalculate only those days extracted (not all between oldest and
%   newest).
...
for i=1:length(filesToDownload)
        ...
        % if the zip made it across, add the zip file to the progress list
        if exist(fullfile(pth_zip,fnstr),'file')
            filesProcessProgressList = [ filesProcessProgressList; fnstr ];
        end
...
if ~exist('date_HF','var')
    ...
    if exist('date_HF','var')
        ...
        %new_calc_and_save(calc_date,siteId); % Dec 4, 2012
        path(pth_old);
    end
end
