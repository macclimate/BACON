function [cal_values_new,cal_values] = extract_LI7000_calibrations(dateIn,configIn,instrumentNum,overwriteCal_flag,do_plots)
...
%                     overwriteCal_flag==0)
% do_plots          - create troubleshooting plots
%
...
%                               Last modification:  Oct  1, 2013

% Revisions:
% 
% Oct  1, 2013 (Zoran)
%   - Introduced an ini file parameter: Instrument.Cal.HaarFilterLength
%     (used to be a constant N=400).  Old programs that don't have this
%     field will default to 400.
%   - In the line below, added coeff *0.9 to avoid edge effects. That
%     improved Ambient Averages when calibrations run at the begining of
%     half hour and there are not enough points to average.
%            indAvgAmbient_before = 1:ind_cal0*0.90;
%   - Limited the search of Cal1 end point to 3 x length of Cal0 cycle:
%     From the program:
%       Limit the number of points searched for the peak to avoid
%       finding random changes in CO2 anywhere in the entire trace. 
%        Old: ind_tmp = round(ind_cal1+N/2+DurationCal0/3:length(c));
%        New: ind_tmp = round(ind_cal1+N/2+DurationCal0/3:3*DurationCal0);
%
% Oct 27, 2011 (Zoran)
%   - added do_plots as an input parameter (default is 0: no plots)
%   - improved detection of cal1 start by using absolute values of xcorr
%   - added xcorr plot
%   - chaged plots
% Mar 23, 2011
...
if isfield(configIn.Instrument(instrumentNum).Cal,'ExtractType')
    if strcmp(upper(configIn.Instrument(instrumentNum).Cal.ExtractType),'MANUAL')
        ...
        if isfield(configIn.Instrument(instrumentNum).Cal,'HaarFilterLength')
            N          = configIn.Instrument(instrumentNum).Cal.HaarFilterLength;
        else
            fprintf('Setup file field: "Instrument.Cal.HaarFilterLength" is missing.\n')
            fprintf('Using FilterLength = 400\n');
            N = 400;
        end
        ...
        % didn't want to change it but didn't want to lose the algorithm
        % either (Z):
        % xc_ind = find(xl+N/2>0);
        % xc_ind=xc_ind(1:end-N);
        % lags_vector = xl(xc_ind)+N/2;
        % c = xc(xc_ind);
        ...
        if ind_cal0 > lenavg+buf
            ...
            % added coeff *0.9 to avoid edge effects (Zoran, Oct 1, 2013)
            indAvgAmbient_before = 1:ind_cal0*0.90;
        end
        ...
        %
        % Note #1 (Zoran - 20111026): skip also 1/3 of the CAL0 length.  There should
        %   be no valid spikes in this area.
        % Note #2: To avoid problems regarding measuring ambient air
        %   use abs(c)
%         ambco2=mean(EngUnits_tmp(indAvgAmbient_before,1));
%         if configIn.cal1 > ambco2
%           [minCorr, ind_cal2] = min(c(ind_cal1+N/2+DurationCal0/3:end));
%           ind_cal2 = ind_cal2 + ind_cal1+N/2+DurationCal0/3;
%         else
%            [minCorr, ind_cal2] = max(c(ind_cal1+N/2+DurationCal0/3:end));
%           ind_cal2 = ind_cal2 + ind_cal1+N/2+DurationCal0/3; 
%         end

%       Limit the number of points searched for the peak to avoid
%       finding random changes in CO2 anywhere in the entire trace. (Zoran, Oct 1,
%       2013)
%         ind_tmp = round(ind_cal1+N/2+DurationCal0/3:length(c));
        ind_tmp = round(ind_cal1+N/2+DurationCal0/3:3*DurationCal0);
        [minCorr, ind_cal2] = max(abs(c(ind_tmp)));
        ind_cal2 = round(ind_cal2 + ind_cal1+N/2+DurationCal0/3); 


        % store duration of cal1 cycle
        %CAL1
        ...
        if do_plots
            ...
            figure(1)
            ind_plt = 1:length(EngUnits_tmp(:,1));
            plot(ind_plt,EngUnits_tmp(:,1),...
                ind_plt(indAvgAmbient_before),EngUnits_tmp(indAvgAmbient_before,1),'ro',...
                ind_plt(indAvgCal0),EngUnits_tmp(indAvgCal0,1),'ro',...
                ind_plt(indAvgCal1),EngUnits_tmp(indAvgCal1,1),'ro',...
                ind_plt(indAvgCal2),EngUnits_tmp(indAvgCal2,1),'ro',...
                ind_plt(indAvgAmbient_after),EngUnits_tmp(indAvgAmbient_after,1),'ro' ...                
            )
            ax=axis;
            line([ind_cal0 ind_cal0],ax(3:4),'color','r');
            line([ind_cal1 ind_cal1],ax(3:4),'color','m');
            line([ind_cal2 ind_cal2],ax(3:4),'color','g');
            line(ind_plt(indAvgAmbient_before([1 end])),mean(EngUnits_tmp(indAvgAmbient_before,1))* [1 1],'color','k','linewidth',4)                        
            line(ind_plt(indAvgCal0([1 end])),mean(EngUnits_tmp(indAvgCal0,1))* [1 1],'color','k','linewidth',4)                                            
            line(ind_plt(indAvgCal1([1 end])),mean(EngUnits_tmp(indAvgCal1,1))* [1 1],'color','k','linewidth',4)                                            
            line(ind_plt(indAvgCal2([1 end])),mean(EngUnits_tmp(indAvgCal2,1))* [1 1],'color','k','linewidth',4)                                
            line(ind_plt(indAvgAmbient_after([1 end])),mean(EngUnits_tmp(indAvgAmbient_after,1))* [1 1],'color','k','linewidth',4)            
            title('CO2');


            figure(2)
            plot(ind_plt,EngUnits_tmp(:,2),...
                ind_plt(indAvgAmbient_before),EngUnits_tmp(indAvgAmbient_before,2),'ro',...
                ind_plt(indAvgCal0),EngUnits_tmp(indAvgCal0,2),'ro',...
                ind_plt(indAvgCal1),EngUnits_tmp(indAvgCal1,2),'ro',...
                ind_plt(indAvgCal2),EngUnits_tmp(indAvgCal2,2),'ro',...
                ind_plt(indAvgAmbient_after),EngUnits_tmp(indAvgAmbient_after,2),'ro' ...                
            )
            ax=axis;
            line([ind_cal0 ind_cal0],ax(3:4),'color','r');
            line([ind_cal1 ind_cal1],ax(3:4),'color','m');
            line([ind_cal2 ind_cal2],ax(3:4),'color','g');
            line(ind_plt(indAvgAmbient_before([1 end])),mean(EngUnits_tmp(indAvgAmbient_before,2))* [1 1],'color','k','linewidth',4)                        
            line(ind_plt(indAvgCal0([1 end])),mean(EngUnits_tmp(indAvgCal0,2))* [1 1],'color','k','linewidth',4)                                            
            line(ind_plt(indAvgCal1([1 end])),mean(EngUnits_tmp(indAvgCal1,2))* [1 1],'color','k','linewidth',4)                                            
            line(ind_plt(indAvgCal2([1 end])),mean(EngUnits_tmp(indAvgCal2,2))* [1 1],'color','k','linewidth',4)                                
            line(ind_plt(indAvgAmbient_after([1 end])),mean(EngUnits_tmp(indAvgAmbient_after,2))* [1 1],'color','k','linewidth',4)            
            title('H2O')
...
if isfield(cal_values,'Ignore') %#ok<NODEF>
