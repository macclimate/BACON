...
for d = 1:length(dates)
    ...
    if strcmp(upper(loc),'CR SITE')
        ...
        pth_ch = '\\Fluxnet02\HFREQ_CR\MET-DATA\Csi_net\';
        fileName    = [pth_ch 'CH_21X_' num2str(doy) '.ch'];
        try 
            C = dlmread([fileName],',');
            disp(['Successfully read ' fileName]);
        catch 
            pth_ch = '\\Fluxnet02\HFREQ_CR\MET-DATA\Csi_net\CH_21X_cor\';
            fileName    = [pth_ch 'CH_21X.' num2str(doy)];
            try
                C = dlmread([fileName],','); 
                disp(['Successfully read ' fileName]);
    ...
    for i        = 1:nbr_sample;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%% dub in 2m TC for bad data
        ind_beg = datevec(datenum(2005,5,22));
        ind_end = datevec(datenum(2005,6,24));
        if tt(i) > datenum(ind_beg) & tt(i) < datenum(ind_end)
           pth_2        = biomet_path(yy(1),'cr','climate\clean\');
           tv_2          = read_bor(fullfile(pth_2,'clean_tv'),8);
           ind_pick = find(tv_2==fr_round_hhour(tt(i))); 
           Air_Temp(i)  = read_bor(fullfile(pth_2,'snow_temperature_20cm'),[],[],[],ind_pick);
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if strcmp(upper(loc),'CR SITE')
        ...
        pth_out = '\\ANNEX001\CONTRIBUTIONS\PaulJassal\cr_ch_recalc';
        %pth_out = '\\ANNEX001\HFREQ_CR\MET-DATA\hhour\';
        %%%%pth_out = '\\FLUXNET02\HFREQ_CR\MET-DATA\hhour\';
    end
