...
elseif ismember(SiteId,{'HP09' 'HP11'})
    ...
    ind_plt(6) = find(strcmp(variableNames, {'temperature_avg_csat_logger'}));
...
plot(tv(ind),trace_str(ind_plt(6)).data(ind),'r','linewidth',2)
...
if ~ismember(SiteId,{'HP09' 'HP11' 'MPB1' 'MPB2' 'MPB3'})
    ...
    plot(tv(ind),trace_str(ind_plt(5)).data(ind),'Color',[0 0.502 0],'linewidth',2)
    set(gca,'XLim',[tv(ind(1)) tv(ind(end))]);
    set(gca,'XTick',tick_val);
    set(gca,'YLim',[-30 30]);
...
% plot monthly GEP
ylim_gep = pl_single_panel(DOY,GEP_FIG,GEP_MONTHLY,'MTH',legstr,1,'g C m^{-2}',[],2,4,'GEP_{mon}',[.75 .9],10,8,10);
