...
%%    [PC_name,SiteLoc] = fr_get_pc_name;

    if exist('pth')~=1 | isempty(pth)
%%        if strcmp(upper(SiteLoc),'MCM')                                 % if in MacMillan
            if exist('\\paoa001\sites\') == 7                           % and the PAOA001 is present
                switch upper(SiteID)                                    % use one of the preset
                    case 'CR', pth = '\\paoa001\sites\CR\hhour\';       % paths for each site
                    case 'PA', pth = '\\paoa001\sites\PAOA\hhour\';
                    case 'BS', pth = '\\paoa001\sites\PAOB\hhour\';
                    case 'JP', pth = '\\paoa001\sites\PAOJ\hhour\';
                    otherwise,
                end
%%            else                                                    % else use local path
%%                [junk, pth] = FR_get_local_path;                    % 
%%            end
        else                                                            % assume that program is runing at a
            [junk, pth] = FR_get_local_path;                            % site computer and use the local path
        end
    end
    %if exist('pth')~=1 | isempty(pth)
    %     c= fr_get_init(SiteID,dateIn);                         % get initialization variables for the give date,
    %     pth = c.hhour_path;                                    % site and use the local path
    %elseif pth(end)~='\'
    %    pth = [pth '\'];
    %end
