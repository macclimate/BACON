...
% Revisions:
% June 12, 2014
%   -FCRN format file exporting for CCRN set to six months back from
%   current month (had been set to 3).
% Nov 9, 2010
%   -for some reason, flag_six_months disappeared from the argument list in
%   July 2010. Nick fixing his own error. 
% July 2, 2010
%   -checks rounded tv length for hhourly freq.. if not hhourly, don't
%   round to the nearest hhour (to preserve higher or lower averaging freq)
%-------------------------------------------
% Check input
%-------------------------------------------
...
if length(traces(1).timeVector) <=17568 & length(traces(1).timeVector) >=17520 % to nearest half-hour
    clean_tv = fr_round_hhour(traces(1).timeVector);
else
    clean_tv = traces(1).timeVector; % to handle other averaging frequencies (e.g. 3min, 10min, hourly, etc.)
end
%                                                      
%                                                     
