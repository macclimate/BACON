...
try
   ...
   SiteId = Selection.Site;
   Year = str2double(char(Selection.Year));
   GMTshift = Selection.GMTshift;
   ftype = Selection.ftype;
   pathFlag = Selection.Path;
   if ismember(pathFlag,[2 3])
       DISmirrorloc = Selection.DISmirrorloc;
   end
...
if pathFlag == 2
     out_inf = fcrnexport_dir(SiteId,pathFlag,ftype,DISmirrorloc);  
elseif ismember(pathFlag,[ 0 1 ])
     out_inf = fcrnexport_dir(SiteId,pathFlag,ftype);
else
     out_inf.pth = DISmirrorloc;
end
trace_str = [];
...
for i=1:length(trace_str)
    ...
    trace_str(i).DOY = trace_str(i).DOY+GMTshift;
end
set(hObject,'UserData',trace_str);
...
%     trace_str(ind).data(ind_999) = data_mean;
    ...
    dv = datevec(trace_str(1).TimeVector);
...
%    plot(trace_str(ind).DOY(ind_nonan),trace_str(ind).data(ind_nonan));
    ...
    site_and_year = [SiteId ' ' num2str(dv(1))];
    text(0,0,site_and_year,...
        'VerticalA','top',...
        'Units','centimeter',...
