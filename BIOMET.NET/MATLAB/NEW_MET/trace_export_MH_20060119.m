...
if ~strcmp(lower(file_opts.out_path), 'database')
   if isfield(trc,'data') & ~isempty(trc.data) & length(trc.data)>1
       ...
       case 'bnc' 
           ...
           % Here we deal with the generic case - clean_tv for hhourly data
           % and with the MH site HF data (clean_3min_tv). Any other special
           % cases will have to be hard coded here.
           ...
           elseif  length(clean_tv) == 175200 | length(clean_tv) == 175680
              ...
              save_bor([path 'clean_unknown_tv'],8,clean_tv);
           end
       case 'bnr'
