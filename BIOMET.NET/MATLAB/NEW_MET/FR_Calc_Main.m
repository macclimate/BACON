function [Stats,HFout] = fr_calc_main(SiteFlag,startDate,endDate,HFexport);
%
%
%
%
...
% Revisions:
%   June 3, 2010 (Nick)
%       - modified LI-7000 SN# check to reflect new analog configuration
%       protocol (SN=700x)
%   April 9, 2010 (Nick)
%       - added input option/flag for HF export for a single hhour
%   May 13, 2008 (Christian/Nick/Zoran)
%       - Added an implemenatation of c.Delays.UseFlag (overwriting of the
%       setup delay with the calculated delays)
%   Oct 18, 2007
...
%                     FR_convert_to_eng_units(RawData_DAQ,2,c);   
       ...
       if  c.licor > 7000 & c.licor <= 7999     % Oct 18, 2007  removed the siteFlag check because a LI7000 was installed at BS (Nick)                                                          
            disp(['LI-7000 data detected... Configuration (see licor.m)) = ' num2str(c.licor)]);
            [EngUnits_DAQ,chi,test_var] = ...
        ...
        if length(EngUnits_DAQ) > max(c.Delays.ArrayLengths)
            ...
            for Del_i = 1:length(c.Delays.Channels)
                ...
                %% Added the implementation of c.Delays.UseFlag
                %% (Zoran @20080513)
                if  Del_i<= 2 & c.Delays.UseFlag(Del_i)==1
                    c.Delays.All(Del_i) = DelTimes(Del_i);
                end
            end
        end                  
    else
    ...
    %Nick, April 9, 2010
    % if used has asked for HF export for the hhour, do it here
    if nargin ==4 
        if nargout==2 & HFexport==1
          HFout = EngUnits_DAQ;
        end
    end
