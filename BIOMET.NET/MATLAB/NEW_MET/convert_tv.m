function [converted_tv,year,day,hhmm] = convert_tv(tv,output_format,gmt_shift,...
...
%    day                 - contains the day for each element in converted_tv
%    hhmm                - contains the hour and minute for each element in converted_tv, 
...
case 'mtv'
   converted_tv = tv; 
   day = floor(converted_tv - TimeOffset);
otherwise
...
% Round time vector to seconds
tmp = round((converted_tv - floor(converted_tv))*86400) / 86400;
H  = floor(24 *(tmp - floor(tmp)));
MI = floor(60 *(24 *(tmp - floor(tmp))-H));
hhmm = 100 * H + MI;
