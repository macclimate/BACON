...
% Last Modified by GUIDE v2.5 07-Mar-2006 17:33:04
...
out_inf = fcrnexport_dir(SiteId,pathFlag);
trace_str = [];
...
%    plot(trace_str(ind).DOY(ind_nonan),trace_str(ind).data(ind_nonan));
    plot(trace_str(ind).DOY,trace_str(ind).data);
    line(trace_str(ind).DOY(ind_999),trace_str(ind).data(ind_999),...
        'LineStyle','none','Marker','.','Color','r');
    line(trace_str(ind).DOY(ind_nan),trace_str(ind).data(ind_nan),...
