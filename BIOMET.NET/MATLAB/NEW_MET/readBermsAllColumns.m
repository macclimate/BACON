...
%  May 19, 2010
%   -modified dlmread syntax to handle matlab 6 to 7 migration (Nick)
...
% check user's MATLAB version--dlmread handles consecutive spaces
% differently in matlab7!
verstr=x(ind).Version; 
if strcmp(verstr(1),'7')
    sectionRead = dlmread(fileName,'',2,0);
else
    sectionRead = dlmread(fileName,' ',2,0);
end
