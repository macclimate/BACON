...
%                     FR_convert_to_eng_units(RawData_DAQ,2,c);   
        ...
        %                        *data collected digitally but processed via DAQ algorithms       
        if (strcmp(upper(SiteFlag),'PA') & currentDate >= datenum(2006,11,20,12,0,0) & currentDate < datenum(2022,1,0))
            [EngUnits_DAQ,chi,test_var] = ...
                FR_convert_to_eng_units(RawData_DAQ,99,c);  % used a new DAQ_SYS_TYPE parameter 99
        else
            [EngUnits_DAQ,chi,test_var] = ...
                FR_convert_to_eng_units(RawData_DAQ,2,c);
        end
