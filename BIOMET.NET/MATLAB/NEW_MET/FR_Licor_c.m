...
% ----------------------**added Feb 19/07---------------------------------
%                       5 - without any corrections (added to handle the
%                           case of a dummy LI-7000 whose "voltages" are
%                           already corrected)
% --------------------------------------------------------------
...
% Revisions:
%   Feb 19, 2007
%       -added flagX = 5 to prevent all corrections for happening in the case
%       that the data input are voltages from an LI-7000 instrument (Nick)
%	July 11, 2003
%		- corrected the line:
...
if ~ismember(3,flagX) & ~ismember(5,flagX)
    v = v .* Pcorr;                                 % do the pressure correction
end

%if flagX ~= 2                                       % if the flagX says so
if ~ismember(2,flagX) & ~ismember(5,flagX)
...
if ~ismember(4,flagX) & ~ismember(5,flagX)
    x = x .* Tcorr;                                 % do the temperature correction
end

%if flagX ~= 1                                       % if the flagX says so
if ~ismember(1,flagX) & ~ismember(5,flagX)
