function [t,x] = MH_pl(ind, year, select, fig_num_inc, pause_flag)
...
if year >= 2005
    pth = biomet_path(year,'MH','cl');      % get the climate data path
    pth_MET1 = fullfile(pth,'MH_Clim');
    pth_Transect = fullfile(pth,'MH_Transect');
    axis1 = [340 400];
    axis2 = [-10 5];
    axis3 = [-50 250];
    axis4 = [-50 250];
...
t=read_bor(fullfile(pth_MET1,'MH_Clim_dt'));            % get decimal time from the data base
%t_TM = read_bor(fullfile(pth_Transect,'MH_Transect_160_dt')); % time vector for MH Transect: soil T and M (30 min output)
t_PAR = read_bor(fullfile(pth_Transect,'MH_Transect_150_dt')); % time vector for MH Transect PAR measurements (3 min output)
...
trace_path  = str2mat(fullfile(pth_MET1,'Batt_Volt_AVG'),fullfile(pth_Transect,'Batt_Volt_AVG'),fullfile(pth_Transect,'Phone_Vlt_AVG'),fullfile(pth_MET1,'MH_Clim.34'));
trace_legend= str2mat('V_{clim}','V_{trans}','V_{phone trans}','V_{phone clim}');
trace_units = 'Volts';
y_axis      = [12 13];
fig_num = fig_num + fig_num_inc;
%x = plt_msig( trace_path, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
x = plt_msig( trace_path, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num,[ 1; 1; 1; 13.2/8.55 ],[ 0; 0; 0; 0 ]);
...
trace_path  = str2mat(fullfile(pth_Transect,'SoilM_1_AVG'),...
                      ...
                      fullfile(pth_Transect,'SoilM_8_AVG'),...
...
%----------------------------------------------------------
% Soil Moisture Probe Gain Corrections (EC-20s)
%----------------------------------------------------------

% check probes 12-17 (EC-20's) which are excited together with and without
% 1-11, then calculate gain correction

for ec20=12:17,  
    sm1 = read_bor(fullfile(biomet_path(2007,'MH'),['Climate\MH_Transect\SoilM_' num2str(ec20) '_AVG']));
    sm2 = read_bor(fullfile(biomet_path(2007,'MH'),['Climate\MH_Transect\SoilM2_' num2str(ec20) '_AVG']));
    fig_num = fig_num + fig_num_inc;
    figure(fig_num);
    set(fig_num,'menubar','none',...
            'numbertitle','off',...
            'Name',['Output V for soilM' num2str(ec20) ]);
    plot(t,sm1(ind),'r-',t,sm2(ind),'b-');
    legend('Ex','No Ex');
    title(['Output V for soilM' num2str(ec20) ]);
    zoom on;
    if pause_flag == 1;pause;end
    fig_num = fig_num + fig_num_inc;
    figure(fig_num);
    set(fig_num,'menubar','none',...
            'numbertitle','off',...
            'Name',['Gain correction for soilM' num2str(ec20) ]);
    plot(t,sm1(ind)./sm2(ind),'g-');
    title(['Gain correction for soilM' num2str(ec20) ]);
    zoom on;
    if pause_flag == 1;pause;end
end

%----------------------------------------------------------
% Soil Moisture Profile Centre
%----------------------------------------------------------

try
    trace_name  = 'Transect/Diagnostics: Soil Moisture Profile Centre';
    trace_path  = str2mat(fullfile(pth_Transect,'SoilM_19_AVG'),...
        fullfile(pth_Transect,'SoilM_20_AVG'),...
        fullfile(pth_Transect,'SoilM_21_AVG'));
    trace_legend= str2mat('20 cm','40 cm','80 cm');
    trace_units = 'mV';
    y_axis      = [0 1000];
    fig_num = fig_num + fig_num_inc;
    x = plt_msig( trace_path, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
    ...
    %----------------------------------------------------------
    % Soil Moisture Profile South Edge
    %----------------------------------------------------------
    
    trace_name  = 'Transect/Diagnostics: Soil Moisture Profile South Edge';              
    trace_path  = str2mat(fullfile(pth_Transect,'SoilM_22_AVG'),...
        fullfile(pth_Transect,'SoilM_23_AVG'),...
        fullfile(pth_Transect,'SoilM_24_AVG'));
    ...
    %----------------------------------------------------------
    % Soil Moisture Profile South 15 m
    %----------------------------------------------------------
    
    trace_name  = 'Transect/Diagnostics: Soil Moisture Profile South 15m';
    trace_path  = str2mat(fullfile(pth_Transect,'SoilM_25_AVG'),...
        fullfile(pth_Transect,'SoilM_26_AVG'),...
        fullfile(pth_Transect,'SoilM_27_AVG'));
    ...
    %----------------------------------------------------------
    % Soil Moisture Profile South 30 m
    %----------------------------------------------------------
    
    trace_name  = 'Transect/Diagnostics: Soil Moisture Profile South 30cm';                  
    trace_path = str2mat(fullfile(pth_Transect,'SoilM_28_AVG'),...
        fullfile(pth_Transect,'SoilM_29_AVG'),...
        fullfile(pth_Transect,'SoilM_30_AVG'));
    ...
    %----------------------------------------------------------
    % Soil Moisture Probe Gain Corrections (EC-5s)
    %----------------------------------------------------------
    
    for ec5=[19 22 25]  
        sm1 = read_bor(fullfile(biomet_path(2007,'MH'),['Climate\MH_Transect\SoilM_' num2str(ec5) '_AVG']));
        sm2 = read_bor(fullfile(biomet_path(2007,'MH'),['Climate\MH_Transect\SoilM2_' num2str(ec5) '_AVG']));
        fig_num = fig_num + fig_num_inc;
        figure(fig_num);
        set(fig_num,'menubar','none',...
            'numbertitle','off',...
            'Name',['Output V for soilM' num2str(ec5) ]);
        plot(t,sm1(ind),'r-',t,sm2(ind),'b-');
        legend('Ex','No Ex');
        title(['Output V for soilM' num2str(ec5) ]);
        zoom on;
        if pause_flag == 1;pause;end 
        fig_num = fig_num + fig_num_inc;
        figure(fig_num);
        set(fig_num,'menubar','none',...
            'numbertitle','off',...
            'Name',['Gain correction for soilM' num2str(ec5) ]);
        plot(t,sm1(ind)./sm2(ind),'g-');
        title(['Gain correction for soilM' num2str(ec5) ]);
        zoom on;
        if pause_flag == 1;pause;end
    end
