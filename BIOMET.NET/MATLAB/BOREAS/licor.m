...
%                                          Last modification:  Apr 14, 2012
%                                          
                                           

% Revisions:
% Apr 14, 2012 (Zoran)
%      - Combined Alta's licor.m changes with UBC's.  It should now work for both groups.
% June 29, 2010
%       - added Li6262 sn587001 owned by UNBC
% June 3, 2010
%       - began new LI-7000 protocol (700x replaces 7000) to reflect
%       changes in h2o analog span
% June 11, 2008
%       -switched coeffs for #1037001 with #1037 to follow convention of the most recent
%        calibration having SN with the extra three digits. Also adjusted
%        BS_init_all where 1037 is deployed in the profile TCH to match
%        this change. (Nick)
%  Nov 16, 2007
%       - added #1037001
%  Nov 7, 2007
...
elseif num == 674001
    if strcmp(CalDate,'new') | strcmp(CalDate,'2007-12-13')
        % Calibration date: 13 dec 2007
        cp = [  2.35277e-17  -4.02621e-13 4.78154e-9  1.19136e-5   0.13616 0];
        hp = [  8.55651e-12   2.83287e-6  6.23753e-3  0 ];
        Tc = 40.05;
        Th = 40.60;
        pp = [0.01536 59.312];     % s/n PX 201 (Jan 12, 1996)
    end
...
%     elseif strcmp(CalDate,'1999-03-22')
       % Calibration date: Mar 22, 1999
        ...
        pp = [0.01536 58.919];     % s/n PX 617
%      end
elseif num == 1036002 % same licor as 1036, just different calibration (Oct 29, 2007)
        cp = [  2.67462e-17  -4.67586e-13 5.21808e-9  1.00636e-5   0.138043 0];
        hp = [  -5.33834e-12   2.68708e-6  6.18363e-3  0 ];
        Tc = 41.07;
        Th = 41.09;
        pp = [0.01536 58.919];     % s/n PX 617
% Calibration date: Mar 24, 1999
elseif num == 10362 % same licor as 1036002, but prevents integer overflow error in UBC_DAQ
        cp = [  2.67462e-17  -4.67586e-13 5.21808e-9  1.00636e-5   0.138043 0];
        hp = [  -5.33834e-12   2.68708e-6  6.18363e-3  0 ];
        Tc = 41.07;
        Th = 41.09;
...
elseif num == 1038    
 ...
 elseif num == 7002    
    %if strcmp(CalDate,'new') | strcmp(CalDate,'1998-02-19')
       ...
       % to 120 kPa, for 0 to 5V
        cp = [  800/5000 0];
        hp = [  50/5000  0 ]; % new biomet standard range on h2o analog channel
        Tc = 36.6;
        Th = 41.9;
        % pp = [0.03062 59.23];     
        pp = [60/5000 60];
...
elseif num == 587001
    if strcmp(CalDate,'new') | strcmp(CalDate,'2000-01-01')
       % Calibration date: ?????
        cp = [  3.8290E-17  -6.5100E-13 5.9395E-09  9.3590E-06  1.4144E-01 0];
        hp = [  1.3955E-12   2.8514E-06  6.1070E-03  0 ];
        Tc = 39.53;
        Th = 39.96;
        pp = [0.01515 55.530];     % s/n PX ????
    end
    
% Altaf's LI-6262
%  
% Calibration date: Feb 12, 2007
elseif num == 228001
    cp = [ 1.49276e-17  -2.85374e-13  3.6688e-9   1.42518e-5   0.143782  0];
    hp = [ 1.21651e-11  2.88602e-6    6.31028e-3   0];
    Tc = 42.38;
    Th = 43.02;
    pp = [0.01547 60.768];
    

% Factory Re-calibration, Calibration date: May 08, 2009
elseif num == 228002 
    cp = [ 3.89485e-17  -6.77605e-13  5.62987e-9   8.88548e-6   0.138303  0];
    hp = [ 2.20451e-11  2.85472e-6    5.59357e-3   0];
    Tc = 34.66;
    Th = 34.33;
    pp = [0.01549 61.184];  
