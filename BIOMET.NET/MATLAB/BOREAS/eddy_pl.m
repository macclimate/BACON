...
%                           Last modification:  Jan 11, 2008
%

% Revisions:
%   Jan 11, 2008
%       -Nick changed BS RMYoung to read raw MSC data (from CR7 datalogger)
%   Oct 12, 2006
%       -changed RMYoung trace location to BERMS\ALL from BERMS\AL1
...
%        t_RMYoung = read_bor([pthClim 'fr_a\fr_a_tv'],8)-yearOffset-GMTshift;
   ...
   case {'PA','BS','JP'},traceName = 'squeeze(stats.AfterRot.AvgMinMax(1,[14 16 12],:))';
        switch upper(siteID)
            ...
            case 'BS'
                %pthBERMS = biomet_path(year,siteID,'BERMS\all'); % Nick changed to \ALL from \AL1, Oct. 12, 2006
                pthBERMS = biomet_path(year,siteID,'Climate\BS_cr7_3'); % Nick changed to read raw MSC data (CR7) Jan 11, 2008
                try
                    %RMYoung = read_bor([pthBERMS 'Wind_Spd_AbvCnpy_26m']);
                    RMYoung = read_bor([pthBERMS 'BS_3.51']);
                    %t_RMYoung = read_bor([pthBERMS 'clean_tv'],8)-yearOffset-GMTshift;
                    t_RMYoung = read_bor([pthBERMS 'BS_3_tv'],8)-yearOffset-GMTshift;
                catch
...
try
    ...
    ylabel('kPa')
    switch upper(siteID)
       case 'CR', legend('2 m','12 m','27 m','42 m') 
       case 'PA', legend('0.5 m','2 m','4 m','10 m','18 m','22 m','28 m','39 m') 
       case 'BS', legend('0.3 m','1 m','3 m','8 m','10 m','15 m','22 m','25 m')
    end
