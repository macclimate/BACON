...
T_CNR1  = read_bor([pthoy 'OY_ClimT\T_CNR1_AVG'],[],[],years,indOut);
...
plot(t,[T_CNR1 HMP_T PT_100]);
legend('T_{CNR1}','T_{HMP}','T_{PRT}');
xlabel('DOY');
ylabel('\circC');
title({'CNR1 temperature'});
h = gca;
set(h,'YLim',Tax,'XLim',[st ed]);
%%datetick('x','dd');
zoom on;
...
CNR1_net_calc = L_upper - L_lower + S_upper - S_lower ;
if years>=2003
    plot(t,[CNR1_net S_upper S_lower  L_upper+LongWaveOffset L_lower+LongWaveOffset  CNR1_net_calc]);
    legend('Net','S upper','S lower','L upper','L lower', 'NET_{calc}');
else
    plot(t,Rn);    
end
xlabel('DOY');
