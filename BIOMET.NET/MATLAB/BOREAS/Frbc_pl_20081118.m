...
%                           Last modification:  Sept 26, 2008
%
...
% title(['WaterQ: ' vstr ]);
% axis([t(1) t(end) 11 14]);
% ylabel('Voltage (V)');
% xlabel('DOY');
% legend('AVG','MAX','MIN');
% set(gcf,'Name','WaterQ 12V Battery Voltage')
% grid on;zoom on; 
...
%----------------------------------------------------------
% MPS Battery Voltage (12V)
%----------------------------------------------------------
trace_name  = 'MPS Logger Battery Voltage';
trace_path  = str2mat([pth 'MPS\Batt_Volt_AVG'],[pth 'MPS\Batt_Volt_MAX'],[pth 'MPS\Batt_Volt_MIN'] );
trace_legend = str2mat('MPS_{AVG}','MPS_{MAX}','MPS_{MIN}');
trace_units = '(V)';
y_axis      = [11 14];
fig_num = fig_num + fig_num_inc;
x = plt_msig( trace_path, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
...
%----------------------------------------------------------
% MPS Logger Panel Temperature
%----------------------------------------------------------
trace_name  = 'MPS Logger Panel Temp';
trace_path  = str2mat([pth 'MPS\Panel_Tmp_AVG'],[pth 'MPS\Panel_Tmp_MAX'],[pth 'MPS\Panel_Tmp_MIN'] );
trace_legend = str2mat('AVG','MAX','MIN');
trace_units = '(degC)';
y_axis      = [5 25] - WINTER_TEMP_OFFSET;
fig_num = fig_num + fig_num_inc;
...
trace_path  = str2mat(fr_logger_to_db_fileName(ini_climMain, 'Press_Ref_AVG', pth),...
                      ...
                      fr_logger_to_db_fileName(ini_climMain,'Pchamber_AVG', pth));
trace_legend = str2mat('Ref_R','Ref_L','Cal0','Cal1','Chamb');
trace_units = '(psi)';
y_axis      = [0 2600];
fig_num = fig_num + fig_num_inc;
