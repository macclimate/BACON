...
%        
    ...
    case 'LGR1'
        ...
        Rainfall = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\LGR1_AGGP_Clim\','Rainfall'),[],[],year,ind);
        Rainfall_cum = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\LGR1_AGGP_Clim\','Rainfall_Tot'),[],[],year,ind);
...
xlabel('DOY')
h = gca;
set(h,'XLim',[st ed+1],'YLim',[370 445])
title({'LGR: ';'CO_2'});
set_figure_name(SiteID)
...
%-----------------------------------------------
% HMP Air Temp and Humidity
%-----------------------------------------------

fig = fig+1;figure(fig);clf;

[ax,h1,h2] = PLOTYY(tv,HMP_T, tv, HMP_RH);
set(get(ax(1), 'YLabel'), 'String', 'T \circC')
ylim(ax(1),[-5 20])
set(get(ax(2), 'YLabel'), 'String', 'RH %')
ylim(ax(2),[50 100])
xlim([st ed+1]);
grid on;zoom on;xlabel('DOY')
title({'Eddy Correlation: ';'HMP_{T} and HMP_{RH}'})
set_figure_name(SiteID)
ax = axis; line([ed ed],ax(3:4),'color','y','linewidth',5);
...
%-----------------------------------------------    
fig = fig+1;figure(fig);clf;    
plot(tv, [VWC_grass_5 VWC_grass_10 VWC_grass_30 VWC_grass_60],... 
...
xlabel('DOY')
h = gca;
set(h,'XLim',[st ed+1],'YLim',[0.20 0.60]);
title({'LGR: ';'Soil Moisture'});
set_figure_name(SiteID)
ylabel('\Theta_V')
ax = axis; line([ed ed],ax(3:4),'color','y','linewidth',5);
legend('Grass_{5}', 'Grass_{10}', 'Grass_{30}', 'Grass_{60}','Row_{5}', 'Row_{10}','Row_{30}','Row_{60}', 'location','northwest') 
...
%-----------------------------------------------
fig = fig+1;figure(fig);clf;    
[ax,h1,h2] = plotyy(tv, Rainfall, tv, Rainfall_cum); 
