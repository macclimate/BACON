...
if ~ECnewLoc
  ...
  trace_legend= str2mat('Avg','Min','Max','Avg_{oldloc}','Min_{oldloc}','Max_{oldloc}');
end
trace_units = '(\circC)';
y_axis      = [0 40]-WINTER_TEMP_OFFSET;
fig_num = fig_num + fig_num_inc;
...
if ~ECnewLoc
  ...
  trace_legend= str2mat('HMP Tair','Sonic_{Tc}','Sonic_{Tc} oldloc');
end
trace_units = '(\circC)';
y_axis      = [0 40]-WINTER_TEMP_OFFSET;
fig_num = fig_num + fig_num_inc;
...
if ~ECnewLoc
  trace_path  = str2mat(fullfile(pthFl,'Instrument_2.Avg_1'),fullfile(pthFl,'Instrument_2.Min_1'),...
   ...
   trace_path  = str2mat(fullfile(pthEC,'Instrument_3.Avg_1'),fullfile(pthEC,'Instrument_3.Min_1'),...
                fullfile(pthEC,'Instrument_3.Max_1'),fullfile(pthFl,'Instrument_2.Avg_1'),fullfile(pthFl,'Instrument_2.Min_1'),...
                fullfile(pthFl,'Instrument_2.Max_1'),fullfile(pthFl,'Instrument_6.Avg_1'));
            trace_legend= str2mat('CO2_{Avg}','CO2_{Min}','CO2_{Max}','CO2_{AvgOldLoc}','CO2_{MinOldLoc}','CO2_{MaxOldLoc}','Boreal GasFinder');
end
fig_num = fig_num + fig_num_inc;
x = plt_msig( trace_path, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
if pause_flag == 1;pause;end
...
try
    GF = read_bor(fullfile(pthFl,'Instrument_6.Avg_1'));
    if ~ECnewLoc
    ...
    disp('Problem plotting 1:1 of LI-7200 vs. GasFinder')
end
%----------------------------------------------------------
% H2O mixing ratio
%----------------------------------------------------------
...
%----------------------------------------------------------
trace_name  = ['SQ-RMY81000 wind vector components'];
if ~ECnewLoc
...
Pbar= read_bor(fullfile(pthFl,'Instrument_5.Avg_3'));
Pcell_old=read_bor(fullfile(pthFl,'Instrument_2.Avg_6'));
Pcell=read_bor( fullfile(pthLgr,'Irga_P_Avg'));

Pgauge_old=Psite-Pcell_old;
Pgauge=Psite-Pcell;
...
y_axis      = [0 10];
fig_num = fig_num + fig_num_inc;
x = plt_msig( [Pgauge Pgauge_old], ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
if pause_flag == 1;pause;end
...
if ~ECnewLoc
 ...
 trace_legend = str2mat('Fc','Fc oldloc');
 trace_units = '(\mumol CO_{2} m^{-2} s^{-1})';
 y_axis      = [-10 15];
 fig_num = fig_num + fig_num_inc;
 x = plt_msig( [ Fc_new Fc_hf ], ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
end
...
if ~ECnewLoc
  ...
  trace_units = '(W m^{-2} )'; 
  trace_legend= str2mat('LE','LE oldloc');
  y_axis      = [-50 400];
  fig_num = fig_num + fig_num_inc;
  x = plt_msig( [ LE_new LE_hf], ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
end
...
if ~ECnewLoc
  ...
  trace_units = '(W m^{-2} )'; 
  trace_legend= str2mat('H','H oldloc');
  y_axis      = [-50 400];
  fig_num = fig_num + fig_num_inc;
  x = plt_msig( [ H_new H_hf], ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
end
