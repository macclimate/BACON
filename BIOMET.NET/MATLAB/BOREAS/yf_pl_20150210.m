...
fig_num = fig_num + fig_num_inc;
if year(1)>= 2013,
    x = plt_msig( trace_path, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num,[1 1 1 15] );
    h=get(gcf,'chi');
    for ind_h = 1:length(h);
        if strcmp(get(h(ind_h),'tag'),'legend');p1=get(h(ind_h),'pos');end;
    end;
...
trace_path  = str2mat(  fr_logger_to_db_fileName(ini_climMain, 'Batt_Volt_AVG', pth),...
                        ...
                        fullfile(pth_CPEC,'batt_volt_Avg'));
trace_legend = str2mat('Avg','Max','Min','CPEC200');
trace_units = '(V)';
y_axis      = [11.0 12.5];
fig_num = fig_num + fig_num_inc;
x = plt_msig( trace_path, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
