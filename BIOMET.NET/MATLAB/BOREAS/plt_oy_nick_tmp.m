...
%
%
% E.Humphreys   July 11, 2000           Last modified: Jan 16, 2008
%
% Revisions:
% Jan 16, 2008 - Nick changed precip/rainfall plots so that TBRG1 and TBRG2
%                are plotted rather than an average of the two
...
% Nick changed Jan 16, 2008
cumrain1 = cumsum(P1_all);
cumrain2 = cumsum(P2_all);
...
cumrain1   = cumrain1(indOut);
cumrain2   = cumrain2(indOut);
...
title({'Climate: HMP Air Temperature'});
a = legend('HMP_{2.5m}','PT100_{2.5m}');
set(a,'Visible','off');
h = gca;
...
%----------------------------------------------------------
fig = fig+1;figure(fig);clf;
plot(t,cumrain1,'b-',t,cumrain2,'r-',t,cumprecip,'g-');
xlabel('DOY');
ylabel('(mm)');
title({'Climate: Cumulative Precipitation'});
zoom on;
a = legend('TBRG1','TBRG2','Geonor',2);
set(a,'visible','on');
h = gca;
set(h,'YLim',[min([cumrain;cumprecip])-10 max([cumrain;cumprecip])+10],'XLim',[st ed]);
%%datetick('x','dd');
if pause_flag == 1;pause;end

%----------------------------------------------------------
% Cumulative Precipitation - Cumulative TBRG
%----------------------------------------------------------
fig = fig+1;figure(fig);clf;
plot(t,cumprecip-cumrain);
xlabel('DOY');
ylabel('(mm)');
title({'Climate: Cumulative Precip - Cumulative TBRG'});
zoom on;
h = gca;
...
title({'Climate: Radiation'});
a = legend('Q-dw','Q-uw','S-LI-dw','S-Ki-dw','S-Ki-uw');
set(a,'visible','off');
h = gca;
