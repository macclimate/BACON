function Stats = fr_calc_main(SiteFlag,startDate,endDate,w_bar)
%
%
% THIS IS ONLY A TEST PROCEDURE TO CHECK THE EFFECTS OF W_BAR ON
% AFTER ROTATION FLUXES
%
%
%
...
for i = 1:hhours
    ...
    EngUnits_DAQ(12,:) = EngUnits_DAQ(12,:) + w_bar;
...
EngUnits_GillR2(3,:) = EngUnits_GillR2(3,:) + w_bar;
...
if m2 > 1 & n2 >= 16
    ...
    direction   = fr_Gill_wind_direction(EngUnits_DAQ([14 16],:), ...
                                           c.sonic) + c.Orientation;% wind direction DAQ
    direction   = rem(direction,360);
    MiscX(7)    = mean(direction);
...
elseif strcmp(c.site,'CR') & m1 > 1
    ...
    direction   = fr_Gill_wind_direction(EngUnits_GillR2([1 2],:), ...
                                           c.sonic) + c.Orientation;% wind direction GillR2
    direction   = rem(direction,360);   
    MiscX(5)    = mean(direction);
    MiscX(6)    = std(direction);
end


%===============================================================
function direction   = fr_Gill_wind_direction(wind,sonicType) 
...
if strcmp(c.site,'PA')
    ...
    if BarometricP < 89 | BarometricP > 105                 % check if the pressure makes sense
        BarometricP = 94.5;                                 % default pressure for CR is 94.5kPa
    end
    BarometricP = StatsRotated.AvgMinMax(1,20);             % barometric pressure
