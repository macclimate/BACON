...
%          zoom on; pause;
          ...
          SHFP1 = read_bor(fullfile(biomet_path('yyyy','HDF11'),'Climate\FR_Clearcut','Sheat_flux_Avg1'),[],[],year,ind);
          SHFP2 = read_bor(fullfile(biomet_path('yyyy','HDF11'),'Climate\FR_Clearcut','Sheat_flux_Avg2'),[],[],year,ind);
          SHFP3 = read_bor(fullfile(biomet_path('yyyy','HDF11'),'Climate\FR_Clearcut','Sheat_flux_Avg3'),[],[],year,ind);
          SHFP4 = read_bor(fullfile(biomet_path('yyyy','HDF11'),'Climate\FR_Clearcut','Sheat_flux_Avg4'),[],[],year,ind);
%         SHFP4 = read_bor([fr_logger_to_db_fileName(ini_climMain, 'SHFP4_AVG', pthc)],[],[],year,ind);
%         SHFP5 = read_bor([fr_logger_to_db_fileName(ini_climMain, 'SHFP5_AVG', pthc)],[],[],year,ind);
%         SHFP6 = read_bor([fr_logger_to_db_fileName(ini_climMain, 'SHFP6_AVG', pthc)],[],[],year,ind);
...
if  ~isempty(c.System(nMainEddy).Delays.Samples)
    h = line([t(1) t(end)],c.System(nMainEddy).Delays.Samples(1)*ones(1,2));
    set(h,'color','y','linewidth',1.5)
    h = line([t(1) t(end)],c.System(nMainEddy).Delays.Samples(2)*ones(1,2));
...
if  ~isempty(c.System(nMainEddy).Delays.Samples)
    ax=axis;
    h = line(c.System(nMainEddy).Delays.Samples(1)*ones(1,2),ax(3:4));
...
if  ~isempty(c.System(nMainEddy).Delays.Samples)
    ax=axis;
    h = line(c.System(nMainEddy).Delays.Samples(2)*ones(1,2),ax(3:4));
    set(h,'color','y','linewidth',2)
else
  if strcmp(upper(SiteID),'YF') % Nick added Oct 8, 2009
        ax=axis;
