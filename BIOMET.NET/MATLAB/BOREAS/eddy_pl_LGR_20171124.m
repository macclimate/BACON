...
%        
    ...
    case 'LGR1'
        ...
        tv    = tv(ind);
         ...
         Batt_logger = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','BattV_Avg'),[],[],year,ind);
         Batt_logger_min = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','BattV_Min'),[],[],year,ind);
         Batt_logger_max = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','BattV_Max'),[],[],year,ind);
         Batt_logger_std = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','BattV_Std'),[],[],year,ind);
         
         Ptemp_logger = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','PTemp_Avg'),[],[],year,ind);
         Ptemp_logger_std = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','PTemp_Std'),[],[],year,ind);

         T_BigPump_fan = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','TC_BigPump_fan_Avg'),[],[],year,ind);
         T_BigPump_fan_std = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','TC_BigPump_fan_Std'),[],[],year,ind);
         
         T_SmallPump_fan = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','TC_SmallPump_fan_Avg'),[],[],year,ind);
         T_SmallPump_fan_std = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','TC_SmallPump_fan_Std'),[],[],year,ind);
         
         T_Hut_fan = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','TC_Fan_Avg'),[],[],year,ind);
         T_Hut_fan_std= read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','TC_Fan_Std'),[],[],year,ind);
         
         T_LGR_fan = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','TC_LGR_fan_Avg'),[],[],year,ind);
         T_LGR_fan_std = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','TC_LGR_fan_Std'),[],[],year,ind);
         
         Fan_Status = read_bor(fullfile(biomet_path('yyyy',SiteID),'Climate\','FanPortStatus_Avg'),[],[],year,ind);
...
xlabel('DOY')
h = gca;
set(h,'XLim',[st ed+1],'YLim',[20 50])
title({'LGR: ';'Pump Temperatures'});
set_figure_name(SiteID)
ylabel('\circC')
ax = axis; line([ed ed],ax(3:4),'color','y','linewidth',5);
legend('LGR Pump','','','LI7000 Pump') 
