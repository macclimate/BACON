...
if strcmp(upper(SiteID),'YF') & st>=datenum(2015,7,9,18,0,0);
...
if strcmp(upper(SiteID),'YF') & datenum(year,1,st)>=datenum(2015,7,9,18,0,0);
    ...
    %-----------------------------------------------
    fig = fig+1;figure(fig);clf;
    plot(t,[Tbench_in Tbench_in_Min Tbench_in_Max]);
    grid on;zoom on;xlabel('DOY')
    %h = gca;
    %set(h,'XLim',[st ed+1], 'YLim',[-1 22])
    ...
    plot(t,[means_enc(:,[6]) maxAll_enc(:,6) minAll_enc(:,6)]);
    grid on;zoom on;xlabel('DOY')
    h = gca;
    set(h,'XLim',[st ed+1], 'YLim',[-1 22])
    title({'Eddy Correlation LI-7200: ';'H_2O '})
    set_figure_name(SiteID)
    ylabel('(mmol mol^{-1} of dry air)')

    legend('LI7200_{avg}','LI7200_{max}','LI7200_{min}',-1)
    zoom on;
    ...
    fig = fig+1;figure(fig);clf;
    %plot(t,[means(:,[5]) maxAll(:,5) minAll(:,5)],tv,co2_GH);
    plot(t,[means_enc(:,[5]) maxAll_enc(:,5) minAll_enc(:,5)]);
    legend('LI7200_{avg}','LI7200_{max}','LI7200_{min}');
    grid on;zoom on;xlabel('DOY')
    h = gca;
    set(h,'XLim',[st ed+1], 'YLim',[300 500])
    title({'Eddy Correlation LI-7200: ';'CO_2'})
    set_figure_name(SiteID)
    ylabel('\mumol mol^{-1} of dry air')
