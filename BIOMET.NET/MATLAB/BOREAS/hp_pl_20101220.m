...
%----------------------------------------------------------
trace_name  = 'Climate/Diagnostics: Fc WPL-corrected'; 
...
[Fc_wpl, E_wpl] = apply_WPL_correction(c_w(ind,:), c_v(ind,:), T_w(ind,:), cc(ind,:), cv(ind,:), T(ind,:), P(ind,:));  %unrotated
[Fc_rot, E_rot] = apply_WPL_correction(wc_rot, wH_rot, wT_rot, cc(ind,:), cv(ind,:), T(ind,:), P(ind,:));  %rotated

trace_legend= str2mat('CO_{2} WPL 2 rotations (raw)', 'CO_{2} WPL unrotated (raw)');
trace_units = '(\mumol CO_{2} m^{-2} s^{-1})';
y_axis      = [-10 15];
fig_num = fig_num + fig_num_inc;
x = plt_msig( [Fc_rot Fc_wpl],1:length(ind), trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
...
trace_legend= str2mat('H_{2}O WPL 2 rotations (raw)', 'H_{2}O WPL unrotated (raw)');
trace_units = '(mmol m^{-2} s^{-1})';

fig_num = fig_num + fig_num_inc;
y_axis      = [-1 5];
x = plt_msig( [E_rot E_wpl], 1:length(ind), trace_name, trace_legend, year, trace_units, y_axis, t, fig_num );
