...
switch user_data_in.type
case 'xcorr'
   ind = find(abs(user_data_in.xdata) <= 5);
...
case '1to1'
   trace_1 = user_data_in.data(:,val(1));
   trace_2 = user_data_in.data(:,val(2));
   ...
   set(g,'XGrid','on','YGrid','on');
   title_str   = [''];
   xlabel_str	= [char(user_data_in.titles(val(1))) ' ' char(user_data_in.units(val(1)))];
   ylabel_str	= [char(user_data_in.titles(val(2))) ' ' char(user_data_in.units(val(2)))];
