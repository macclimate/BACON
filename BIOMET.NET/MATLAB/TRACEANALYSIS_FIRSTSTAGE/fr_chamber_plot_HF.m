...
% Revisions:
% Sept 14, 2007
%   -modified so that fr_set_site will be used to set network paths if the
%   fr_current_siteId does not match SiteId (Nick)
...
% modified to ensure correct paths are used: Nick Sept 14, 2007
if ~strcmp(upper(SiteFlag),upper(fr_current_siteid))
   fr_set_site(SiteFlag,'network');
end

% tell the user where the program is looking for HF data, Nick Sept 14, 2007
disp(['...Looking for chamber HF data in ' char(fr_get_local_path) ]);
