...
% kai* Feb 12, 2003                     Last modified: July 10, 2007 
%
% Revisions:
% June 4, 2008
%   -Nick modified so that automated_cleaning.log can be created for
%   cleaned trace export
% July 10, 2007
%   -If stages contains a 6 all cleaning stages are run and web graphs are
...
for i = 1:n
    ...
    for j = 1:m
        yy = Years(j);
        yy_str = num2str(yy(1));
        if ~strcmp(db_out,db_pth)
            try
                diary(fullfile(db_out,[ SiteId '_automated_cleaning.log']));
            catch
                disp(lasterr);
            end
        else % 
    ...
    % June 4/08: Nick added so that cleaning log file can be created locally in db_out
    if ~strcmp(db_out,db_pth)
        cmdstr = ['move ' fullfile(db_out,[SiteId '_automated_cleaning.log'])... 
                   ' ' fullfile(db_out,yy_str,SiteId,[ 'automated_cleaning.log'])];
        dos(cmdstr);
    end
end
