...
% File created: June 6, 2008 (Nick)         Last revision: Feb 2, 2017
% 
% Revisions:
% Feb 2, 2017 (Zoran)
%   - Removed Nick from the email list
%   - Updated the reference data base to
%       '\\annex001\clean_db_backups\20170203\database\';
%   - Removed compressions
% Jan 30, 2012
%	-finall added HP09 and HP11
% Oct 8, 2010
%   -added MPB3
% Oct 6, 2010
...
dv=datevec(now);
arg_default('sites',{'CR' 'HDF11' 'YF' 'OY' 'BS' 'PA' 'JP' 'HJP02' 'HJP75' 'MPB1' 'MPB2' 'MPB3' 'HP09' 'HP11'});
...
for m=1:length(sites)
    ...
    All_Sites     = {'BS'   'CR' 'HDF11'  'HJP02' 'HJP75' 'JP'   'OY' 'PA'   'YF' 'MPB1' 'MPB2' 'MPB3' 'HP09' 'HP11'};
    All_Site_name = {'PAOB' 'CR' 'HDF11'  'HJP02' 'HJP75' 'PAOJ' 'OY' 'PAOA' 'YF' 'MPB1' 'MPB2' 'MPB3' 'HP09' 'HP11'};
   ...
   % cd(base_dir); % This causes the cleaning log file to be created in base_dir
    ...
    [s,mes] = copyfile(fullfile(biomet_path('Calculation_Procedures','TraceAnalysis_ini',[SiteId '\Derived_Variables']),'*.*'),fullfile(pth_ini,'Derived_Variables'));
    if ~isempty(mes)
        disp(mes)
        ...
        disp(sprintf('%s ...Derived Variables folder done ',datestr(now)));
...
%------------------------------------------------------------
% Feb 3, 2017 (Z)
% Removed all compression. It takes forever to decompress and 
% there should be enough space of ANNEX001 to store uncompressed backups
% (~60GB/year, at one backup per month)
%------------------------------------------------------------
