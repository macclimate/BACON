...
for k = 3:1:length(d)
    ...
    for i = 3:1:length(tmp_d)
        tic;
        [pathstr, tmp_name, tmp_ext, tmp_ver] = fileparts(tmp_d(i).name);
        
        % If it's a .csv file, we'll go to work on it:
        if strcmp(tmp_ext,'.csv')==1
        tmp_fullpath = [tmp_dir tmp_name tmp_ext];    
            %%%% try to extract the year from the filename %%%%%%%%%%%%%%%
            tmp_uscr = strfind(tmp_name,'_');
            tmp_uscr = [0 tmp_uscr length(tmp_name)];
            tmp_diff = diff(tmp_uscr);
            tmp_fives = find(tmp_diff==5);
            %     tmp_yr = '';
            for j = 1:1:length(tmp_fives)
                tmp_to_check = tmp_name(tmp_uscr(tmp_fives(j))+1:(tmp_uscr(tmp_fives(j)+1))-1);
        ...
        disp(['Successfully read ' tmp_name tmp_ext ]);
        end
