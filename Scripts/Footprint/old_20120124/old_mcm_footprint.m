...
flag_file = NaN.*ones(length(data.Year),1);
max_fetch = NaN.*ones(length(data.Year),1);
Xpct_all = NaN.*ones(length(data.Year),10);
%%% Constants:
% colors for plotting:
...
Xcrit = floor(Xcrit.*100)./100;
stab_thresh = 0.04; % stability threshold
Xpct_index = [(1:1:10)' (0.50:0.05:0.95)'];
...
for year = year_start:1:year_end
    ...
    try
        ...
        temp_flag_file(Xpct(:,col_to_use) <=temp_max_fetch) = 1;
        % Case 2: When the Xpct is more than max fetch (remove data)
        temp_flag_file(Xpct(:,col_to_use) > temp_max_fetch) = NaN;
        
        
        flag_file(data.Year == year) = temp_flag_file;
