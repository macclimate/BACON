...
function [Fc,L,xp,F2H,zL,max_contour]=jjb_mcm_footprint_hsieh(ustar,H,Ta,z,h,max_fetch)
...
max_contour = NaN.*ones(length(zL),1);
...
for j = 1:1:length(zL)
    ...
    if isnan(temp_Fc)==1; max_contour(j,1) = NaN; else
        ind3 = find(x <= max_fetch(j,1),1,'last');
        if isempty(ind3)==1
          max_contour(j,1) = 0; 
        else
          max_contour(j,1) = temp_Fc(ind3);
        end
    end
    ctr = 1;
%     if j >= 70;
