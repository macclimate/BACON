function [flag_file, max_fetch, Xpct_all] = jjb_mcm_footprint(data, site, year_start, year_end, fp_type, Xcrit, ustar_thresh, plot_flag)
% mcm_footprint.m
...
flag_file = NaN.*ones(length(data.Year),size(Xcrit,1));
max_fetch = NaN.*ones(length(data.Year),1);
Xpct_all = NaN.*ones(length(data.Year),size(Xcrit,1));
...
Xpct_index = [(1:1:size(Xcrit,1))' Xcrit];
...
for year = year_start:1:year_end
    ...
    try
        ...
        % Calculate maximum fetch available for each hhour:
        ...
        switch fp_type
            case 'Schuepp'
                [x_max_m, x_max_h, xf_m, xf_h,L,zL] = mcm_Schuepp_footprint(u, ustar, H, Ta, RH, APR, z, h,z,Xcrit);
                x_max = x_max_h; % Distance of maximum contribution to measured flux
                Xpct  = xf_h; % Fetch distances for given % of measured flux

            case 'Hsieh'
                [Fc,L,xp,F2H,zL,WFFP] = jjb_mcm_footprint_hsieh(ustar,H,Ta,z,h,temp_max_fetch);
        ...
        switch fp_type
             case 'Hsieh'
                 max_fetch(data.Year == year) = WFFP;
                 Xpct_all(data.Year == year, :) = NaN;
            otherwise
        ...
        switch plot_flag
            case 'on'
                ...
                try
                    figure(1);clf;
                    hwr = wind_rose(W_Dir_deg(~isnan(W_Dir_deg.*u)), u(~isnan(W_Dir_deg.*u)),'percbg', 'None');
