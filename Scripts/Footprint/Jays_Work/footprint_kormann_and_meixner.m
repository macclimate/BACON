function [phi,f,D_y,xs,ys,param] = footprint_kormann_and_meixner(z_0,z_m,u,sig_v,L,x_max,y_max,d);
...
arg_default('x_max',1000);
arg_default('y_max',400);
arg_default('d',5);
...
phi = reshape(phi.*d^2,length(xs),length(ys));
D_y = reshape(D_y.*d,length(xs),length(ys));
f   = reshape(f.*d,length(xs),length(ys));
