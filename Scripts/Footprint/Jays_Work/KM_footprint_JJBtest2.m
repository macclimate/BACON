function KM_footprint_JJBtest2
...
%         [phi,f,D_y,xs,ys,param] = footprint_kormann_and_meixner_bob(z_0,z_m,u_tmp,ust_tmp,sig_v_tmp,L(i,1),y_max,x_max,pix);
        [phi,f,D_y,xs,ys,param] = footprint_kormann_and_meixner_bob(z_0,z_m,u_tmp,ust_pred(i,1),sig_v_tmp,L(i,1),y_max,x_max,pix);
%         phi = real(phi);phi(isnan(phi))=0;
        ...
        text(min(xx_mask(:))+10, max(yy_mask(:))-50,{['pbar = ' num2str(inputs(i,1))], ['wdpolar = ' num2str(inputs(i,2))], ['u = ' num2str(inputs(i,3))],...
            ...
            ['LE = ' num2str(inputs(i,7))], ['sigv = ' num2str(inputs(i,8))],['ust = ' num2str(inputs(i,9))], ['L = ' num2str(L(i,1))],['L* = ' num2str(Lstar(i,1))]},...
        ...
        if sp_ctr == 5;
            figure(f1);set(gcf,'visible','off')
            print('-dpng',['/home/brodeujj/Matlab/Data/KM_footprint_test/JJBtest2-figs/fp_run_' num2str(i-3) '-' num2str(i)])
            clf;
            figure(f2);set(gcf,'visible','off')
            print('-dpng',['/home/brodeujj/Matlab/Data/KM_footprint_test/JJBtest2-figs/cumfp_run_' num2str(i-3) '-' num2str(i)])
            clf;
            sp_ctr = 1;
        end
...
save([output_path 'KMtest2_' num2str(year) '_results.mat'],'out');
end