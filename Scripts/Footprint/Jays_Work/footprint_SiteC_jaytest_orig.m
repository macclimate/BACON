...
function footprint_SiteC_jaytest_orig(year)
...
for i = 1:1:size(ind_data,1)
    if ~isnan(ind_data(i,1))
        ...
        [xsite_rot ysite_rot] = pol2cart(angles_out-wd_polar(i,1)+pi()/2,dist_out/pix);
        ...
        x_plot_rot = repmat((x_min_site_rot*pix:pix:x_min_site_rot*pix+gridsize_rot*pix-1),gridsize_rot,1);
        y_plot_rot = repmat((y_min_site_rot*pix:pix:y_min_site_rot*pix+gridsize_rot*pix-1)',1,gridsize_rot);
...
%         y_y = flipud(y_y);
        ...
        if fig_ctr ==10
            timetag = [num2str(Year(i,1)) create_label(Mon(i,1),2) create_label(Day(i,1),2) '_' create_label(HH(i,1),2) create_label(MM(i,1),2)];
            % Phi within the bounds:
            
            figure(1); subplot(2,2,sp_ctr);
            ...
            if sp_ctr == 5;
                figure(1);
                print('-dpng',['/1/fielddata/Matlab/Data/Flux/Footprint/KM_footprint_test/orig_figs/fp'  timetag '_dp_' num2str(i)])
                clf;
                figure(2);
              print('-dpng',['/1/fielddata/Matlab/Data/Flux/Footprint/KM_footprint_test/orig_figs/cumfp'  timetag '_dp_' num2str(i)])
              clf;
                sp_ctr = 1;
            end
...
% save([output_path 'inbounds_p.mat'],'inbounds_prop');
 out.Xind = Xind;
       ...
       save([output_path 'KM_TP39_' num2str(year) '_results_orig.mat'],'out');
end
%{
