...
for j = 1:1:length(zL)
    ...
    c1=(-1/k/k)*(D1(j,1)*zu(j,1)^P1(j,1)*abs(L(j,1))^(1-P1(j,1)))./x;
    
    temp_Fc(:,1)=exp(c1);
    temp_Fp(:,1)=-(c1./x).*temp_Fc(:,1);
    xp(j,1)=(1/2/k/k)*(D1(j,1)*zu(j)^P1(j,1)*abs(L(j,1))^(1-P1(j,1)));
    F2H=(D1(j,1)/0.105/k/k)*(zm(j,1)^(-1)*abs(L(j,1))^(1-P1(j,1))*zu(j,1)^(P1(j,1)));
