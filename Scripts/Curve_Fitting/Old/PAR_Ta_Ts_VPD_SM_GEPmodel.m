function [GEP_model GEPraw pw coeff] = PAR_Ta_Ts_VPD_SM_GEPmodel(data, RE_in, SM, stdev,  Ustar_th, param_flag, day_constraints)



if nargin == 6 || isempty(day_constraints)
...
%             data.Ustar > Ustar_th & ~isnan(data.VPD) & ~isnan(data.PAR) & ~isnan(data.Ta) & ~isnan(SM));
        ...
        [coeff, Y_hat, R2, sigma] = fitmain([0.03 25 1.12 0.15 1.4 0.7 -2 -.8 8 180], 'fitlogi_GEP2', ...
            [data.PAR(ind_GEP_param) data.Ta(ind_GEP_param) data.Ts5(ind_GEP_param) data.VPD(ind_GEP_param) SM(ind_GEP_param)], ...
            GEPraw(ind_GEP_param), stdev(ind_GEP_param));
...
%             data.Ustar > Ustar_th & ~isnan(data.VPD) &  ~isnan(data.PAR) & ~isnan(data.Ta) & ~isnan(SM) & data.Year == year);
        ...
        [coeff(ctr,:), Y_hat, R2, sigma] = fitmain([0.03 25 1.12 0.15 1.4 0.7 -2 -.8 8 180], 'fitlogi_GEP2', ...
            [data.PAR(ind_GEP_param) data.Ta(ind_GEP_param) data.Ts5(ind_GEP_param) data.VPD(ind_GEP_param) SM(ind_GEP_param)], ...
            GEPraw(ind_GEP_param), stdev(ind_GEP_param));
        ...
        GEP_pred(data.Year == year,1) = (coeff(ctr,1)*coeff(ctr,2)*data.PAR(data.Year == year)./(coeff(ctr,1)*data.PAR(data.Year == year) + ...
            ...
            (1./(1 + exp(coeff(ctr,9)-coeff(ctr,10).*SM(data.Year == year))));
        ctr = ctr+1;
