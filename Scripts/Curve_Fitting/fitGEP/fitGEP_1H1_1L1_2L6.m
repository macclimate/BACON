function out = fitGEP_1H1_1L1_2L6(c_hat,x)
%%
...
if nargin > 1
       y_hat = ((c_hat(1)*c_hat(2)*x(:,1))./(c_hat(1)*x(:,1) + c_hat(2))).* ...
        ((c_hat(3)-c_hat(6))./(1 + exp(c_hat(4).*(c_hat(5)-x(:,2)))) + c_hat(6)) .* ...
        (1./(1 + exp(c_hat(7)-c_hat(8).*x(:,3)))) .* ...
        (1./(1 + exp(c_hat(9)-c_hat(10).*x(:,4))));
    out = y_hat;
else
       y_hat = ((c_hat(1)*c_hat(2)*X(:,1))./(c_hat(1)*X(:,1) + c_hat(2))).* ...
        ((c_hat(3)-c_hat(6))./(1 + exp(c_hat(4).*(c_hat(5)-X(:,2)))) + c_hat(6)) .* ...
...
switch objfun
    ...
    case 'MAWE'
        err = (1./length(X)) .* ( sum(abs(y_hat - Y)./(stdev.^2)));
    otherwise
