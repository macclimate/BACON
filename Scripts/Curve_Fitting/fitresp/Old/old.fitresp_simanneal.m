function [c_hat, y_hat, y_pred, stats, sigma, err] = fitresp_simanneal(coeff_0, funName, Xin, Yin, X_eval, stdev_in, costfun)
...
%     [c_hat, err] = lsqnonlin(funName,coeff_0, [],[], optimset('Algorithm', alg_in, 'MaxFunEvals', 10000,'MaxIter',10000));
% 
% elseif strcmp(min_method,'GN')==1
% %     alg_in = ['''LargeScale''',off, LevenbergMarquardt, off'];
%     [c_hat, err] = lsqnonlin(funName,coeff_0,[],[], optimset('LargeScale','off','LevenBergMarquardt', 'off','MaxFunEvals', 10000,'MaxIter',10000));
% else
%     [c_hat, err] = lsqnonlin(funName,coeff_0,[],[],optimset('MaxFunEvals', 10000,'MaxIter',10000));
% end
