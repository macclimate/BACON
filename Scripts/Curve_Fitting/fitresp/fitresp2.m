function [c_hat, y_hat, y_pred, stats, sigma, err, exitflag, num_iter] = ...
    fitresp2(coeff_0, funName, Xin, Yin, X_eval, stdev_in, options)
%% fitresp.m
...
% Models:
%%% fitresp_1x - L&T
% 1A - Ts only - all unfixed (3 coeffs)
% 1B -  Ts+SM - all unfixed (5 coeffs)
% 1C - Ts - To is fixed (2 coeffs)
% 1D - Ts+SM - To is fixed (4 coeffs)
% 1E - different formulation of 1A (3 coeffs)
% 1F - different formulation of 1B (5 coeffs);
%%% fitresp_2x - Logistic
%
%
%
...
exitflag = 0;
coeff_0_orig = coeff_0;
while exitflag == 0

if strcmp(min_method,'LM')==1
    alg_in = {'levenberg-marquardt',.005};
    [c_hat err resid exitflag output] = lsqnonlin(funName,coeff_0, [],[], optimset('Algorithm', alg_in, 'MaxFunEvals', 10000,'MaxIter',10000, 'TolX',1e-10, 'TolFun', 1e-10, 'Display','off'));
elseif strcmp(min_method,'GN')==1
    %     alg_in = ['''LargeScale''',off, LevenbergMarquardt, off'];
    [c_hat err resid exitflag output] = lsqnonlin(funName,coeff_0,[],[], optimset('LargeScale','off','LevenBergMarquardt', 'off','MaxFunEvals', 10000,'MaxIter',10000, 'TolX',1e-10, 'TolFun', 1e-10, 'Display','off'));
elseif strcmp(min_method,'TRR')==1
    [c_hat err resid exitflag output] = lsqnonlin(funName,coeff_0,[],[],optimset('MaxFunEvals', 10000,'MaxIter',10000, 'TolX',1e-10, 'TolFun', 1e-10, 'Display','off'));
elseif strcmp(min_method,'SA')==1
    eval (['fun_to_use = @' funName ';']);
    [c_hat,err,exitflag,output] = simulannealbnd(fun_to_use,coeff_0,optimset('MaxFunEvals', 30000));
...
if exitflag == 0
    c_adj = coeff_0_orig.* (rand(1,size(coeff_0,2)) + 0.5);
    c_adj(coeffs_to_fix) = coeff_0_orig(coeffs_to_fix);
    coeff_0 = c_adj;
end
end
...
try
    ...
    stats = model_stats(y_hat, Y, [],stdev_in);
    
catch
    disp('Warning (fitresp.m):  Calculating stats failed')
