...
if nargin == 0 || isempty(load_path)==1
    [filename pathname] = uigetfile([ls 'SiteData/*.*'],'Please navigate to directory with conditioned data in it');
...
if ~isempty(data)
    ...
    try
        ...
        %%% Fix a new problem discovered, where a single value gets placed
        %%% in the timestamp somehow- this results in unequal vector
        %%% lengths for time variables
        if ~isequal(size(yyyy,1),size(MON,1),size(DAY,1),size(hh,1),size(mm,1))
            for k = 1:1:length(timestamp)
                clear test_stat;
                test_stat = [str2num(timestamp(k,1:4)) str2num(timestamp(k,6:7)) ...
                    str2num(timestamp(k,9:10)) str2num(timestamp(k,12:13)) ...
                    str2num(timestamp(k,15:16))];
                if size(test_stat,2) == 5
                    ind(k,1) = 1;
                else
                    ind(k,1) = 0;
                end
            ...
            yyyy = yyyy(good_rows); MON = MON(good_rows); DAY = DAY(good_rows);
            hh = hh(good_rows); mm = mm(good_rows);
            data = data(good_rows,:);
        end
