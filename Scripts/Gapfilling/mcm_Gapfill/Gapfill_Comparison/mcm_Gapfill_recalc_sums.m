...
for fp_ctr = 1:1:length(fp_tags) %%%%%%%%%%%%% CHANGE THIS BACK!!!!! %%%%%
    ...
    for uth_ctr = 1:1:length(ustar_th_const)+2
        ...
        for model_ctr = 1:1:length(model_tags)
            ...
            stats(stats_ctr).model = model_tags{model_ctr,1};
            stats(stats_ctr).fp = fp_tags{fp_ctr,1};
            stats(stats_ctr).u_th = ustar_tag;
            sums(stats_ctr).model = model_tags{model_ctr,1};
            sums(stats_ctr).fp = fp_tags{fp_ctr,1};
            sums(stats_ctr).u_th = ustar_tag;           
            sums(stats_ctr).labels = sum_labels;            
            try
            load([save_path 'NEE_GEP_RE/' save_tag '_results.mat']);
            
            %%% Do Stats -- All years and individual years:
            try
            ...
            disp(['Could not load ' save_tag]);
            stats(stats_ctr).all.stats = struct;
            stats(stats_ctr).annual = struct;                
            sums(stats_ctr).sums = struct;
            end
...
save([save_path 'NEE_GEP_RE/recalc_sums.mat'],'sums');