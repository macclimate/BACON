function [final_out f_out] = old_mcm_Gapfill_LRC_NR(data, plot_flag, debug_flag)
...
for i = 1:1:length(centres)
    ...
    [c_hat, y_hat, y_pred, stats, sigma, err, exitflag, num_iter] = ...
        fitGEP([0.01 beta_start(i,1) 0.5 Rref_start(i,1)], 'fitGEP_LRC_NR', X_in ,NEE_temp(~isnan(ind_check)),  X_eval, NEE_std_temp(~isnan(ind_check)), options);
    % predicted values:
    ...
    [c_hat, y_hat, y_pred, stats, sigma, err, exitflag, num_iter] = ...
        fitGEP([0.01 beta_start(i,1) 0.5 Rref_start(i,1) 0], 'fitGEP_LRC_NR_VPD', X_in ,NEE_temp(~isnan(ind_check)),  X_eval, NEE_std_temp(~isnan(ind_check)), options);
    % predicted values:
...
% 1. Any instance of rRMSE being NaN results in coefficients being NaN:
ind_nan1u = find(isnan(stats1u(:,1))); ind_nan2u = find(isnan(stats2u(:,1))); %ind_nan3 = find(isnan(stats3(:,1)));
c_hat1u(ind_nan1u,:) = NaN; c_hat2u(ind_nan2u,:) = NaN; %c_hat3(ind_nan3,:) = NaN;
% 2. If there is less than 10 daytime or nighttime points in any period, make NaN:
ind_nan_pts = find(num_pts_d < 10 | num_pts_n < 10);
...
% c_hat1(ind_bad_stats1,:) = NaN; c_hat2(ind_bad_stats2,:) = NaN; c_hat3(ind_bad_stats3,:) = NaN;
% 4. Find where beta == 50, make NaN to all parameters:
c_hat1u(c_hat1u(:,2)>=100,:) = NaN;c_hat2u(c_hat2u(:,2)>=100,:) = NaN; %c_hat3(c_hat3(:,2)>=49.5,:) = NaN;
% 4. k value less than 0 = 0 (c_hat3 only):
...
c_hat1u(c_hat1u(:,1)>0.3,:) = NaN;c_hat2u(c_hat2u(:,1)>0.3,:) = NaN; %c_hat3(c_hat3(:,2)>=49.5,:) = NaN;
% 6. Curve parameter must be more than 0:
c_hat1u(c_hat1u(:,3)<0,3) = NaN; c_hat2u(c_hat2u(:,3)<0,3) = NaN;
...
for i = 1:1:length(centres)
    ...
    %%%% Estimate for _VPD: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    y_tmp = fitGEP_LRC_NR_VPD(c_hat2(i,1:5),X_eval);
    y_pred2NEE(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp;
    % RE - We'll make alpha = 0, which removes GEP:
    y_tmp = fitGEP_LRC_NR_VPD([0 c_hat2(i,2:5)],X_eval);
    y_pred2RE(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp;
    % GEP - We'll make Rref = 0, which removes RE:
    y_tmp = fitGEP_LRC_NR_VPD([c_hat2(i,1:3) 0 c_hat2(i,5)],X_eval);
    y_pred2GEP(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = -1.*y_tmp;clear y_tmp;
    
    %%%% Estimate for _noVPD -- Unconstrained: %%%%%%%%%%%%%%%%%%%%
    % NEE:
    y_tmp = fitGEP_LRC_NR(c_hat1u(i,1:4),X_eval);
    y_pred1uNEE(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp;
    % RE - We'll make alpha = 0, which removes GEP:
    y_tmp = fitGEP_LRC_NR([0 c_hat1u(i,2:4)],X_eval);
    y_pred1uRE(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp;
    % GEP - We'll make Rref = 0, which removes RE:
    y_tmp = fitGEP_LRC_NR([c_hat1u(i,1:3) 0],X_eval);
    y_pred1uGEP(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = -1.*y_tmp;clear y_tmp;
    
    %%%% Estimate for _VPD -- Unconstrained: %%%%%%%%%%%%%%%%%%%%
    y_tmp = fitGEP_LRC_NR_VPD(c_hat2u(i,1:5),X_eval);
    y_pred2uNEE(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp;
    % RE - We'll make alpha = 0, which removes GEP:
    y_tmp = fitGEP_LRC_NR_VPD([0 c_hat2u(i,2:5)],X_eval);
    y_pred2uRE(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp;
    % GEP - We'll make Rref = 0, which removes RE:
    y_tmp = fitGEP_LRC_NR_VPD([c_hat2u(i,1:3) 0 c_hat2u(i,5)],X_eval);
    y_pred2uGEP(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = -1.*y_tmp;clear y_tmp;
...
title('Final Filled Data - Constained')
subplot(2,1,1); title('NEE');
plot(NEE_pred_final(:,1:2)); hold on;
plot(NEE_clean,'k.')
legend('NoVPD','VPD','measured');
subplot(2,1,2);
hold on;
plot(RE_pred(:,1:2))
h1 = plot(-1.*GEP_pred(:,1:2));
ylabel('GEE                      RE');
legend(h1,'NoVPD','VPD');

f_out(2) = figure('Name', 'Final Filled Data');clf
title('Final Filled Data - Unconstained')
subplot(2,1,1); title('NEE');
plot(NEE_pred_final(:,3:4)); hold on;
plot(NEE_clean,'k.')
legend('NoVPD','VPD','measured');
subplot(2,1,2);
hold on;
plot(RE_pred(:,3:4))
h1 = plot(-1.*GEP_pred(:,3:4));
ylabel('GEE                      RE');
legend(h1,'NoVPD','VPD');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
