function [final_out f_out] = old_mcm_Gapfill_LRC_JJB1(data, plot_flag, debug_flag)
...
if plot_flag == 1;
    ...
    fig_path = [ls 'Matlab/Figs/Gapfilling/LRC_JJB1/' data.site '/'];
...
NEE_tags = {'LRC_JJB1_noVPD';'LRC_JJB1_noVPDall';'LRC_JJB1_VPD'};
...
for i = 1:1:length(centres)
    ...
    %%%%%%%%%%% 1. _noVPD %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    options.costfun =data.costfun; options.min_method ='NMC';%options.f_coeff = []; %
    options.lbound = [1e-2 1e-3  0];    options.ubound = [0.15 50 10];
    if fix_flag == 1
        options.f_coeff = [0.01 NaN NaN];
    else
        options.f_coeff = [];
    end
    clear global;
    [c_hat, y_hat, y_pred, stats, sigma, err, exitflag, num_iter] = ...
        fitGEP([0.01 beta_start(i,1) Rref_start(i,1)], 'fitGEP_Lasslop_noVPD', X_in ,NEE_temp(~isnan(ind_check)),  X_eval, NEE_std_temp(~isnan(ind_check)), options);
    ...
    %%%%%%%%%%% 2. _noVPDall %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    options.costfun =data.costfun; options.min_method ='NMC';options.f_coeff = [];
    options.lbound = [1e-2 1e-3  0 75];    options.ubound = [0.15 50 10 600];
    if fix_flag == 1
    ...
    [c_hat, y_hat, y_pred, stats, sigma, err, exitflag, num_iter] = ...
        fitGEP([0.01 beta_start(i,1) Rref_start(i,1) 400], 'fitGEP_Lasslop_noVPDall', X_in ,NEE_temp(~isnan(ind_check)),  X_eval, NEE_std_temp(~isnan(ind_check)), options);
    % predicted values:
    ...
    %%%%%%%%%%% 3. _VPD %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    options.costfun =data.costfun; options.min_method ='NMC';options.f_coeff = [];
    options.lbound = [1e-2 1e-3  0 0];    options.ubound = [0.15 50 10 10];
    if fix_flag == 1
        options.f_coeff = [0.01 NaN NaN NaN];
    else
        options.f_coeff = [];
    ...
    [c_hat, y_hat, y_pred, stats, sigma, err, exitflag, num_iter] = ...
        fitGEP([0.01 beta_start(i,1) Rref_start(i,1) 0], 'fitGEP_Lasslop_VPD', X_in ,NEE_temp(~isnan(ind_check)),  X_eval, NEE_std_temp(~isnan(ind_check)), options);
    % predicted values:
    ...
    c_hat3(i,1:4) = c_hat;
    clear c_hat y_pred stats options;
    clear global % This line is VERY important; clears hidden global variables in other stacks
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
disp('mcm_Gapfill_LRC_JJB1 done!');
...
% y_pred1b = NaN.*ones(length(wrap_NEE),2); y_pred2b = NaN.*ones(length(wrap_NEE),2);
% y_pred3b = NaN.*ones(length(wrap_NEE),2);
% clear global;
% 
% for i = 1:1:length(centres)
%     %%% Fix for _noVPD:
...
%       % Estimate for _noVPD:
%       y_tmp = fitGEP_Lasslop_noVPD(c_hat1(i,1:3),X_eval);
%       y_pred1b(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp;
%       % Estimate for _noVPDall:
%       y_tmp = fitGEP_Lasslop_noVPDall(c_hat2(i,1:4),X_eval);
%       y_pred2b(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp;
%       % Estimate for _noVPD:
%       y_tmp = fitGEP_Lasslop_VPD(c_hat3(i,1:4),X_eval);
%       y_pred3b(centres(i)-win_size:centres(i)+win_size,rem(i,2)+1) = y_tmp;clear y_tmp; 
%       
% %%% Make a vector the same length as y_pred that records the centre 
% %%% position associated with that window:      
% win_start = centres(i)-win_size; 
...
% %%% Plot the newly predicted NEE for checking:
% f4 = figure();clf;
% subplot(311);plot(wrap_NEE,'k.-');hold on; plot(y_pred1b)
% axis([1 length(y_pred1b) -25 15]);title('noVPD');
% subplot(312);plot(wrap_NEE,'k.-');hold on;plot(y_pred2b);
% axis([1 length(y_pred1b) -25 15]);title('noVPDall');
% subplot(313);plot(wrap_NEE,'k.-');hold on;plot(y_pred3b);
% axis([1 length(y_pred1b) -25 15]);title('VPD');
% print('-djpeg', [fig_path 'model_results']);
% 
% % f10 = figure();clf;
% % subplot(311);
% % plot(y_pred1(:,1),'k');
...
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%% GET FINAL NEE ESTIMATES: %%%%%%%%%%%%%%%%
% 
% %%% Merge the two columns of y_pred data into a single estimated value
% % Do this by weighting the data by its proximity to the centre of the
% % estimation window:
% NEE_pred = NaN.*ones(length(y_pred1b),3);
% 
% centre_dist = [abs(c_loc(:,1) - (1:1:length(y_pred1b))') abs(c_loc(:,2) - (1:1:length(y_pred1b))')];
% total_dist = nansum(centre_dist,2);
% ind_nan1 = find(isnan(centre_dist(:,1))==1);
% ind_nan2 = find(isnan(centre_dist(:,2))==1);
% weights = [(total_dist - centre_dist(:,1))./total_dist (total_dist - centre_dist(:,2))./total_dist ];
% % First, apply the weighted mean measurement into appropriate hhour:
% NEE_pred(:,1) = sum(y_pred1b.*weights,2);
% NEE_pred(:,2) = sum(y_pred2b.*weights,2);
% NEE_pred(:,3) = sum(y_pred3b.*weights,2);
% % Second, if we only have a measurement from one column, use it:
% NEE_pred(ind_nan1,1) = y_pred1b(ind_nan1,2); NEE_pred(ind_nan2,1) = y_pred1b(ind_nan2,1);
% NEE_pred(ind_nan1,2) = y_pred2b(ind_nan1,2); NEE_pred(ind_nan2,2) = y_pred2b(ind_nan2,1);
% NEE_pred(ind_nan1,3) = y_pred3b(ind_nan1,2); NEE_pred(ind_nan2,3) = y_pred3b(ind_nan2,1);
% % Next, need to make a weighting factor:
% f11 = figure();clf;
% h1 = plot(wrap_NEE,'k.-');hold on;
% h2 = plot(NEE_pred); 
% legend(h2, NEE_tags)
% print('-djpeg', [fig_path 'NEE_pred']);
% 
% %%% Trim the datafiles to the appropriate length:
% NEE_pred_final = NEE_pred(win_size+1:length(NEE_pred)-win_size,:);
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%% SAVE THE FINAL NEE DATA and SUMS: %%%%%%%%%%%%%%%%
% 
% for i = 1:1:3
% NEE_fill(isnan(NEE_fill(:,i)),i) = NEE_pred_final(isnan(NEE_fill(:,i)),i);
% num_gaps(i,1) = length(find(isnan(NEE_fill(:,i))));
...
%         ctr = ctr + 1;
%   end
%   master.sum_labels = {'Year';'NEE_filled';'NEE_pred'};
% 
% save([ls 'Matlab/Data/Gapfilling/TP39/Lasslop_JJB1_' NEE_tags{i,1} '_fill_2003-2009.mat'],'master')  
%   
%     clear master
% end
