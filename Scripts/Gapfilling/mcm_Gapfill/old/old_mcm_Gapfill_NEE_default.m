function [] = old_mcm_Gapfill_NEE_default(site,year)
...
save_path = [ls 'Matlab/Data/Flux/Gapfilling/' site '/NEE_GEP_RE/'];
...
default_header = jjb_hdr_read([ls 'Matlab/Data/Flux/Gapfilling/Docs/NEE_Gapfilling_Defaults.csv'],',');
...
for grp = 1:1:length(default)
    ...
    switch default(grp).fp % Apply Schuepp 70% footprint:
        case 'on'
            data_in.NEE = data_in.NEE.*data_in.fp_flag(:,1);
        case 'off' % No change in data
            data_in.NEE = data_in.NEE.*data_in.fp_flag(:,2);
    end
    ...
    fig_tag = ['(NEE_default)_(FP_' default(grp).fp ')_(' ustar_tag ')_(' num2str(yr_start) '-' num2str(yr_end) ')'];
...
fid = fopen([save_path site '_NEE_sums_' datestr(now,29) '.dat'],'w+');
