function [final_out f_out] = old_mcm_Gapfill_LRC_Lasslop(data, plot_flag, debug_flag)
...
for i = 1:1:length(centres)
    ...
    %%% will go crazy:
    options.costfun =data.costfun; options.min_method ='NMC';options.f_coeff = []; %options.f_coeff = [0.05 NaN NaN];
    options.lbound = [-1e6 -1e6 -1e6 50];    options.ubound = [1e6 1e6 1e6 600];
    [c_hat, y_hat, y_pred, stats, sigma, err, exitflag, num_iter] = ...
        fitGEP([0.01 beta_start(i,1) Rref_start(i,1) 250], 'fitGEP_Lasslop_noVPDall', X_in ,NEE_temp(~isnan(ind_check)),  X_eval, NEE_std_temp(~isnan(ind_check)), options);
    % predicted values:
