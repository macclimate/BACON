function [final_out f_out] = mcm_Gapfill_SiteSpec_REpool(data, Ustar_th, plot_flag)
...
%%% Fill the modeled RE data with predicted values:
    RE_model = y_pred(1).RE1all;
RE_model_Tsonly = y_pred(1).RE1all_Tsonly;
...
RE_model = RE_model_Tsonly;


if plot_flag == 1
    %%% Plot the relationships for RE-Ts for each year:
    figure('Name', 'REvsTs');clf;
    ctr = 1;
%     for year = year_start:1:year_end
        f_handle = 'fitresp_3B';
        test_Ts_y =    feval(f_handle,c_hat(1).RE1all,[test_Ts ones(length(test_Ts))]);
        % (c_hat(ctr).RE(1))./(1 + exp(c_hat(ctr).RE(2).*(c_hat(ctr).RE(3)-test_Ts)));
        plot(test_Ts, test_Ts_y,'-','Color', 'k'); hold on;
%         ctr = ctr+1;
%     end
