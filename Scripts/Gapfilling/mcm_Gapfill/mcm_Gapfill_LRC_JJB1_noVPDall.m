function [final_out f_out] = mcm_Gapfill_LRC_JJB1_noVPDall(data, plot_flag, debug_flag)
...
% c_hat2 = NaN.*ones(length(centres),5);c_hat3 = NaN.*ones(length(centres),5);
stats1= NaN.*ones(length(centres),2); 
% stats2= NaN.*ones(length(centres),2);stats3= NaN.*ones(length(centres),2);
y_pred1 = NaN.*ones(length(wrap_NEE),2); 
% y_pred2 = NaN.*ones(length(wrap_NEE),2);y_pred3 = NaN.*ones(length(wrap_NEE),2);
...
%%% Windowed Parameterization:
    ...
    fun_name = 'fitGEP_Lasslop_noVPDall';
        coeffs_use = [1 2 3 4];
...
axis([1 length(y_pred1) -25 15]);title(model_type);
...
c_hat1(isnan(stats1(:,1)),:) = NaN; %c_hat2(ind_nan2,:) = NaN; c_hat3(ind_nan3,:) = NaN;
...
% switch model_type
%     case 'LRC_JJB1_noVPD'
%     case 'LRC_JJB1_noVPDall'
        % 5. Eo value outside of range is equal to end of range (c_hat2 only):
        c_hat1(c_hat1(:,4)>550,4)= 550; c_hat1(c_hat1(:,4)< 100,4)= 100;
%     case 'LRC_JJB1_VPD'
...
% y_pred2NEE = NaN.*ones(length(wrap_NEE),2);y_pred3NEE = NaN.*ones(length(wrap_NEE),2);
y_pred1RE = NaN.*ones(length(wrap_NEE),2); 
% y_pred2RE = NaN.*ones(length(wrap_NEE),2);y_pred3RE = NaN.*ones(length(wrap_NEE),2);
y_pred1GEP = NaN.*ones(length(wrap_NEE),2); 
% y_pred2GEP = NaN.*ones(length(wrap_NEE),2);y_pred3GEP = NaN.*ones(length(wrap_NEE),2);
...
% NEE_pred(:,2) = sum(y_pred2NEE.*weights,2);NEE_pred(:,3) = sum(y_pred3NEE.*weights,2);
GEP_pred(:,1) = sum(y_pred1GEP.*weights,2);
% GEP_pred(:,2) = sum(y_pred2GEP.*weights,2);GEP_pred(:,3) = sum(y_pred3GEP.*weights,2);
RE_pred(:,1) = sum(y_pred1RE.*weights,2);
% RE_pred(:,2) = sum(y_pred2RE.*weights,2);RE_pred(:,3) = sum(y_pred3RE.*weights,2);
...
% save([ls 'Matlab/Data/Gapfilling/TP39/Lasslop_' NEE_tags{1,1} '_fill_2003-2009.mat'],'master')  
  final_out(1).master = master;
  final_out(1).tag = NEE_tags{1,1};

    clear master

disp('mcm_Gapfill_LRC_JJB1_noVPDall done!');
