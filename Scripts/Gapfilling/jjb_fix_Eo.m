function [Eo_weighted Eo_interp] = jjb_fix_Eo(data, debug_flag, plot_flag)
...
if debug_flag ~= 2
    %%%
    Ts_in = data.Ts5; %%% Using 5cm soil temperature as temperature for regression.
    %%%
    RE_raw = NaN.*data.NEE;
...
%         ( data.Ustar >= data.Ustar_th & ~isnan(data.NEE.*data.NEE_std) & ((data.dt < 85 | data.dt > 335) & data.Ts5 < 0.2)  ) );  % non-growing season
    ind_param.RE = find(data.Ustar >= data.Ustar_th & data.RE_flag == 2 & ~isnan(data.Ts5.*data.NEE.*data.NEE_std) & data.NEE >-7);
