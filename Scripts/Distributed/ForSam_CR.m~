...
% if mm == 2007
% b_avg_ideal = jjb_blockavg(PAR1(ideal_GEP), GEP1(ideal_GEP), 200, 70, -25);
% else
b_avg_ideal = jjb_blockavg(PAR1(ideal_GEP), GEP1(ideal_GEP), 100, 70, -25);
% end
...
for kk = 1:1:length(PAR_bottoms)
   %%% These values should be adjusted according to site characteristics:
    ...
    % Bin-average data:
    vpd(kk).bavg = jjb_blockavg(VPD1(vpd_ind,1),GEP_Ts_adj(vpd_ind,1), 0.2, 40, 0);
