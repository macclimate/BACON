...
for year_ctr = year_start:1:year_end
    ...
    %% Get information about the variables (paths and names)
    vars = mcm_get_fluxsystem_info(site, 'varnames', year_ctr);
    %%%%%%%%%%%%%%%
    ...
    for j = 1:1:num_days
        %%% A messy fix for the change to LI7200 and the associated data
        %%% structure changes
        if strcmp(site,'TP39')==1 && year_ctr == 2021 && j == 226
            vars(28).path = 'Instrument(1,1).Avg(5)';
        end
        % disp(datastr(j,:));
        %          if j  == 189
        %          disp('stop. hammer time');
