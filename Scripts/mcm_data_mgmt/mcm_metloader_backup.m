...
if isempty(load_path)
    load_path = uigetdir(loadstart, 'Select the folder of data to process');
end
%% Declaring variables
...
%% DECLARE PATHS
if strcmp(site,'TP74') == true && yr >=2008
    resp = input('Please select file type (<1> = CR10x, <2> = CR1000); ','s');
    if resp == '2'
        ...
        [met_master hdr_cell_tmpt yr_len num_cols] = CR1000_metloader(site, yr_str); % Loads data using CR1000 script.
        skip_flag = 1;
    else
        skip_flag = 0;
    end
...
try
    ...
    if ~isempty(isDUMP) % - If DUMP_Data is in the path of the data:
        ...
        copyfile(pth_data,[dir_data(1:isDUMP-1) 'To_Burn/' site '/' datestr(now,10) datestr(now,11) datestr(now,12) '_' file_data]);
        disp('copying file(s) to the "To_Burn" directory');
        try
            copyfile(pth_data2,[dir_data(1:isDUMP-1) 'To_Burn/' site '/' datestr(now,10) datestr(now,11) datestr(now,12) '_' file_data2]);
        catch
        end
        % Move the file into the /Met/Raw1/ Folder:
        if exist ( [loadstart 'Matlab/Data/Met/Raw1/' site '/' yr_str '/' file_data],'file' ) == 2;
            movefile(pth_data,[loadstart 'Matlab/Data/Met/Raw1/' site '/' yr_str '/' datestr(now,10) datestr(now,11) datestr(now,12) '_' file_data]);
        ...
        try
            if exist ( [loadstart 'Matlab/Data/Met/Raw1/' site '/' yr_str '/' file_data2],'file' ) == 2;
                movefile(pth_data,[loadstart 'Matlab/Data/Met/Raw1/' site '/' yr_str '/' datestr(now,10) datestr(now,11) datestr(now,12) '_' file_data2]);
            else
                movefile(pth_data,[loadstart 'Matlab/Data/Met/Raw1/' site '/' yr_str '/' file_data2]);
            end
