function [RE_flag, GEP_flag] = mcm_GEP_RE_flag(site, PAR, dt, Ts5, Ts2, Ta, GDD,Year)
% mcm_GEP_RE_flag.m
% usage: mcm_GEP_RE_flag(PAR, dt, Ts5, Ta, GDD)
...
switch site
    ...
    case {'TPD','TPAg','TP_VDT'}
        ind_GEP_param = [];
        ind_GEP_model = [];
        for i = min(Year):1:max(Year)
            [gs_params] = params(i, site, 'GS_Dates'); % [gs_start gs_end];
...
%         ind_GEP_model(Year==i) = find(PAR>=15 & (dt>gs_params(1) & dt<=gs_params(2)));
  ...
  clear ind_GEP_param_tmp ind_GEP_model_tmp;
