...
for i=1:length(C)
    ...
    U=get(C(i),'Units'); set(C(i),'Units','Pixels'); Pos=get(C(i),'Position'); set(C(i),'Units',U);
...
if(any(abs(Pos-data.Pos)>1e-3))
    ...
    % Create a real-menubar from the panel UICContextMenu
    CreatMenuBar(uimenuhandle,datahandle,Pos,data.figure_handle)
    drawnow('expose');
end
