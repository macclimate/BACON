...
% Mar 10, 2018
% Incorporated Zoran's changes from Oct 2017 (detailed below)
%%% Oct 18, 2017 (Zoran)
%%%   - Added options for resampling CSAT3 60Hz data: Dates(4) and Dates(5)
...
Dates(2) = datenum(2003,4,14,0,30,0);                     % Change from DumpCOM - Changed from 04-15 to 04-14 by JJB, June 15, 2010
Dates(3) = datenum(2013,2,7,0,30,0);			% Licor motherboard upgraded.  Data output frequency changed
Dates(4) = datenum(2017,3,8,0,0,0);            % Date when CSAT3 started sampling at 60Hz for no good reason
Dates(5) = datenum(2017,10,1,20,0,0);          % Date when CSAT3 stopped sampling at 60Hz (it was reset to do 20Hz 
...
c.site          = 'MCM1';
localPCname	  = fr_get_pc_name; % Added by JJB - Jan 19, 2010
...
c.Instrument(nIRGA).Alignment.ChanNum = 1;
...
elseif dateIn > Dates(4) & dateIn <= Dates(5) 
...
c.Instrument(nCSAT).Alignment.ChanNum = 3;            % chanel used for the alignment (w),  % Changed from (T sonic, ch 4) on Sep 8, 2010 by JJB (from Zoran).
c.Instrument(nCSAT).Alignment.Shift = [0]; 			% added by JJB, July 15, 2010
c.Instrument(nCSAT).Alignment.Span = [-100 100]; 		% added by JJB, July 15, 2010 % Changed to current values [-100 100] on Sep 8, 2010 by JJB on advice of Zoran 
...
c.System(nMainEddy).Delays.RefChan = 3;                         % Delays calculated against this channel (w) % Changed from (T sonic) on Sep 8, 2010 by JJB (from Zoran).
...
c.System(nMainEddy).Alignment.MaxAutoSampleReduction = [3]; 	% added by JJB, July 15, 2010% Change back to 30;
